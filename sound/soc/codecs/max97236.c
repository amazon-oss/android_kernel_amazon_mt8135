...
static int debounce_count = 37;
static int debounce_sleep_duration = 20;
...
static const struct snd_kcontrol_new max97236_snd_controls[] = {
	...
	SOC_SINGLE("Amp HPL Mute Switch", M97236_REG_07_LEFT_VOLUME,
		M97236_MUTEL_SHIFT, M97236_MUTEL_NUM - 1, 0),

	SOC_SINGLE("Amp HPR Mute Switch", M97236_REG_08_RIGHT_VOLUME,
		...
		unsigned int *status_reg)
{
	...
	if (press) {
		if ((status_reg[0] & M97236_MCSW_MASK) ||
			...
			if (adc < M97236_KEY_THRESH_1) {
				...
				string_copy(keystr, "SND_JACK_BTN_5",
						MAX_STRING);
				key = SND_JACK_BTN_5;
			}
		} else {
	...
	max97236->keyscan = key;

	dev_err(max97236->codec->dev, "%s %s\n",
...
static int max97236_jacksw_active(struct max97236_priv *max97236)
{
	...
	int ret;
	if (max97236->pdata->jack_sw_normal_open)
		test_value = 0;
...
static void max97236_delay_before_debounce(struct max97236_priv *max97236)
{
	if (max97236->pdata->jack_sw_normal_open) {
		msleep(800);
		debounce_count = 15;
		debounce_sleep_duration = 10;
	}
}
...
static void max97236_jack_plugged(struct max97236_priv *max97236)
{
	...
	max97236_delay_before_debounce(max97236);
	...
	for (count = 0; count < debounce_count; count++) {
		msleep(debounce_sleep_duration);
		if (!max97236_jacksw_active(max97236))
			goto max97236_jack_plugged_30;
	}
	dev_dbg(max97236->codec->dev, "debounce check finished");
...
static int max97236_probe(struct snd_soc_codec *codec)
{
	...
	if ((request_threaded_irq(max97236->irq, NULL, max97236_interrupt,
		...
		dev_err(codec->dev, "request_irq failed irq=%d\n", max97236->irq);
		ret = -1;
		goto err_access;
	}
	...
	if (pdata->vreg) {
		...
		if (ret)
			dev_err(codec->dev, "Setting regulator voltage failed for regulator err = %d\n", ret);

		ret = regulator_set_optimum_mode(pdata->vreg, 150);
		if (ret < 0)
			dev_err(codec->dev, "Setting regulator optimum mode failed for regulator err = %d\n", ret);
	...
	if (ret)
		...
		snd_jack_set_key(max97236_headset.jack, SND_JACK_BTN_0 | SND_JACK_BTN_3 | SND_JACK_BTN_5, KEY_MEDIA);
...
static int max97236_soc_suspend(struct snd_soc_codec *codec)
{
	...
	if (likely(max97236->pdata && max97236->pdata->clk_enable))
		max97236->pdata->clk_enable(false);
...
static int max97236_soc_resume(struct snd_soc_codec *codec)
{
	...
	if (likely(max97236->pdata && max97236->pdata->clk_enable))
		max97236->pdata->clk_enable(true);
...
int max97236_set_key_div(struct snd_soc_codec *codec, unsigned long clk_freq)
{
	...
	regmap_write(regmap, M97236_REG_12_KEYSCAN_CLK_DIV_HI,
		      (clk_freq / 2000) >> 8);
	regmap_write(regmap, M97236_REG_13_KEYSCAN_CLK_DIV_LO,
		      (clk_freq / 2000) & 0xFF);
	regmap_write(regmap, M97236_REG_14_KEYSCAN_CLK_DIV_ADC, (clk_freq / 200000));
