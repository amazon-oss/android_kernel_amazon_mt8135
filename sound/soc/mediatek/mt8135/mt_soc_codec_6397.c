...
/*#ifdef USING_CLASSD_AMP*/
		if (mCodec_data->mAudio_Amp_class_type == AUDIO_AMP_CLASS_D) {
...
static const char *const speaker_over_current_function[] = { "NoOverCurrent", "OverCurrent" };
static const char *const dac_newif_sck_function[] = { "NotInverse", "Inverse" };
...
static int speaker_oc_flag_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	uint32_t reg_value = Ana_Get_Reg(SPK_CON6);
/*#ifdef USING_CLASSD_AMP*/
	if (mCodec_data->mAudio_Amp_class_type == AUDIO_AMP_CLASS_D) {
...
static int speaker_oc_flag_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int dac_newif_sck_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	uint32_t reg_value = Ana_Get_Reg(AFE_PMIC_NEWIF_CFG2);
	ucontrol->value.integer.value[0] = (reg_value & 0x8000) ? 1 : 0;
	return 0;
}

static int dac_newif_sck_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == 0)
		Ana_Set_Reg(AFE_PMIC_NEWIF_CFG2, 0 << 15, 1 << 15);
	else if (ucontrol->value.integer.value[0] == 1)
		Ana_Set_Reg(AFE_PMIC_NEWIF_CFG2, 1 << 15, 1 << 15);
...
static const struct soc_enum Audio_Amp_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(speaker_over_current_function),
			    speaker_over_current_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(dac_newif_sck_function), dac_newif_sck_function),
};
...
static const struct snd_kcontrol_new mt6397_snd_controls[] = {
	...
	SOC_ENUM_EXT("Speaker_OC_Falg", Audio_Amp_Enum[12], speaker_oc_flag_get,
		     speaker_oc_flag_set),
	SOC_ENUM_EXT("DAC_Newif_Sck_Switch", Audio_Amp_Enum[13], dac_newif_sck_get,
		     dac_newif_sck_set),
};
...
static int dac_sgen_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = mCodec_data->dac_sgen_switch;
	return 0;
}

static int dac_sgen_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	if (mCodec_data->dac_sgen_switch == ucontrol->value.integer.value[0]) {
		pr_debug("%s dummy operation for %u", __func__, mCodec_data->dac_sgen_switch);
		return 0;
	}

	if (ucontrol->value.integer.value[0]) {
		Ana_Set_Reg(ANA_AFE_TOP_CON0, 0x1, 0x1);
		/* Ana_Set_Reg(AFE_SGEN_CFG0, 0x80, 0x80); */
		Ana_Set_Reg(AFE_SGEN_CFG0, 0x4480, 0xff80);
		Ana_Set_Reg(AFE_SGEN_CFG1, 0x0101, 0xffff);
	} else {
		Ana_Set_Reg(ANA_AFE_TOP_CON0, 0x0, 0x1);
		Ana_Set_Reg(AFE_SGEN_CFG0, 0x0, 0x80);
	}

	mCodec_data->dac_sgen_switch = ucontrol->value.integer.value[0];
	return 0;
}

static int adc_sgen_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = mCodec_data->adc_sgen_switch;
	return 0;
}

static int adc_sgen_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	if (mCodec_data->adc_sgen_switch == ucontrol->value.integer.value[0]) {
		pr_debug("%s dummy operation for %u", __func__, mCodec_data->adc_sgen_switch);
		return 0;
	}

	if (ucontrol->value.integer.value[0]) {
		Ana_Set_Reg(ANA_AFE_TOP_CON0, 0x2, 0x2);
		Ana_Set_Reg(AFE_SGEN_CFG0, 0x80, 0x80);
		Ana_Set_Reg(AFE_SGEN_CFG1, 0x0101, 0xffff);
	} else {
		Ana_Set_Reg(ANA_AFE_TOP_CON0, 0x0, 0x2);
		Ana_Set_Reg(AFE_SGEN_CFG0, 0x0, 0x80);
	}

	mCodec_data->adc_sgen_switch = ucontrol->value.integer.value[0];
	return 0;
}

static int dac_freq_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 48000)
		ucontrol->value.integer.value[0] = DAC_FREQ_48000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 44100)
		ucontrol->value.integer.value[0] = DAC_FREQ_44100;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 32000)
		ucontrol->value.integer.value[0] = DAC_FREQ_32000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 24000)
		ucontrol->value.integer.value[0] = DAC_FREQ_24000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 22050)
		ucontrol->value.integer.value[0] = DAC_FREQ_22050;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 16000)
		ucontrol->value.integer.value[0] = DAC_FREQ_16000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 12000)
		ucontrol->value.integer.value[0] = DAC_FREQ_12000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 11025)
		ucontrol->value.integer.value[0] = DAC_FREQ_11025;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] == 8000)
...
static int dac_freq_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int adc_freq_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] == 48000)
		ucontrol->value.integer.value[0] = ADC_FREQ_48000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] == 32000)
		ucontrol->value.integer.value[0] = ADC_FREQ_32000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] == 16000)
		ucontrol->value.integer.value[0] = ADC_FREQ_16000;
	else if (mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] == 8000)
...
static int adc_freq_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static const char *const pmic_sgen_function[] = { "Off", "On" };

static const char *const codec_dl_freq_function[] = {
...
static const char *const codec_ul_freq_function[] = {
...
static const struct soc_enum Pmic_Factory_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(pmic_sgen_function), pmic_sgen_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(pmic_sgen_function), pmic_sgen_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(codec_dl_freq_function), codec_dl_freq_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(codec_ul_freq_function), codec_ul_freq_function),
};
...
static const struct snd_kcontrol_new mt6397_factory_controls[] = {
	...
	Codec_Loopback_Set),
	SOC_ENUM_EXT("DAC_SGen_Switch", Pmic_Factory_Enum[1],
		     dac_sgen_get, dac_sgen_set),
	SOC_ENUM_EXT("ADC_SGen_Switch", Pmic_Factory_Enum[2],
		     adc_sgen_get, adc_sgen_set),
	SOC_ENUM_EXT("DAC_Freq_Switch", Pmic_Factory_Enum[3],
		     dac_freq_get, dac_freq_set),
	SOC_ENUM_EXT("ADC_Freq_Switch", Pmic_Factory_Enum[4],
		     adc_freq_get, adc_freq_set),
};
...
static void TurnOnDmicPower(bool enable)
{
	if (enable) {
		...
		/* AudioMachineDevice */
		Ana_Set_Reg(AUDNVREGGLB_CFG0, 0x0000, 0x0002);
		...
		Ana_Set_Reg(AFE_UL_SRC_CON0_L, 0x0000, 0xffff);
		if (!GetDLStatus()) {
			Ana_Set_Reg(AUDNVREGGLB_CFG0, 0x0002, 0x0002);
			Ana_Set_Reg(AFE_UL_DL_CON0, 0x0000, 0xffff);
		}
...
static const struct soc_enum Audio_UL_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ADC_UL_PGA_GAIN), ADC_UL_PGA_GAIN),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(DMIC_function), DMIC_function),
};
...
static void mt6397_codec_init_reg(struct snd_soc_codec *codec)
{
	...
	Ana_Set_Reg(AFUNC_AUD_CON2, 0x0000, 0x0080);
	Ana_Set_Reg(ZCD_CON0, 0x0101, 0xffff);	/* ZCD setting gain step gain and enable */
	Ana_Set_Reg(AFE_PMIC_NEWIF_CFG2, 1 << 15, 1 << 15);	/* sck inverse */
}
...
static int mtk_mt6397_codec_dev_probe(struct platform_device *pdev)
{
	...
	struct mt_audio_pmic_codec_data *pdata;
	...
	dev_set_drvdata(dev, mCodec_data);

	pdata = dev_get_platdata(&pdev->dev);
	if (pdata)
		mCodec_data->mAudio_Amp_class_type = pdata->mt_audio_board_amp_class_type;
