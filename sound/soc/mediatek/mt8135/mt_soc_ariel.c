...
static struct dentry *mt_sco_pmic_audio_debugfs;
#endif

#define DEBUG_FS_NAME "mtksocaudio"
#define DEBUG_DL_FS_NAME "mtksocdlaudio"
#define DEBUG_PMIC_FS_NAME "mtksocpmicaudio"
...
static ssize_t mt_soc_pmic_debug_read(struct file *file, char __user *buf,
				    size_t count, loff_t *pos)
{
	const int size = 4096;
	char buffer[size];
	...
	pr_notice("mt_soc_pmic_debug_read\n");
	AudDrv_Clk_On();

	n = scnprintf(buffer, size - n, "mt_soc_pmic_debug_read\n");
	n += scnprintf(buffer + n, size - n,
		       "======PMIC digital registers====\n");
	n += scnprintf(buffer + n, size - n, "UL_DL_CON0 = 0x%x\n",
		       Ana_Get_Reg(AFE_UL_DL_CON0));
	...
	/* n+= scnprintf(buffer+n, size-n, "AUDPLL_CON4 = 0x%x\n", AP_Get_Reg(AUDPLL_CON4)); */
	/* n += scnprintf(buffer+n, size-n, "CLK_CFG_9 = 0x%x\n", AP_Get_Reg(CLK_CFG_9)); */
	pr_notice("mt_soc_pmic_debug_read len = %d\n", n);
...
static const struct file_operations mtaudio_pmic_debug_ops = {
	.open = mt_soc_debug_open,
	.read = mt_soc_pmic_debug_read,
};
...
static int mt_soc_ext_speaker_amp_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	/* TODO: change to use pinctrl */
...
static int mt_soc_ext_speaker_amp_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
...
static const struct snd_kcontrol_new mt_soc_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Speaker_Amp_Switch", mt_soc_ariel_enum[1],
		     mt_soc_ext_speaker_amp_get, mt_soc_ext_speaker_amp_set),
};
...
static int mtk_soc_machine_dev_probe(struct platform_device *pdev)
{
	...
	mt_sco_pmic_audio_debugfs = debugfs_create_file(DEBUG_PMIC_FS_NAME,
						      S_IFREG | S_IRUGO, NULL,
						      (void *)DEBUG_PMIC_FS_NAME,
						      &mtaudio_pmic_debug_ops);
#endif
...
static int mtk_soc_machine_dev_remove(struct platform_device *pdev)
{
	...
	debugfs_remove(mt_sco_pmic_audio_debugfs);
#endif
