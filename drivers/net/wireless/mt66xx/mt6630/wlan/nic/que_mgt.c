...
VOID qmDumpQueueStatus(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = TC0_INDEX; i < TC_NUM; i++) {
		DBGLOG(SW4, WARN, ("TC%u ResCount: Max[%02u/%03u] Free[%02u/%03u] PreUsed[%03u]\n",
	...
	DBGLOG(SW4, WARN, ("ToT ResCount: Max[%02u/%03u] Free[%02u/%03u]\n",
...
#if QM_ADAPTIVE_TC_RESOURCE_CTRL
	for (i = TC0_INDEX; i < TC_NUM; i++) {
		DBGLOG(SW4, WARN, ("TC%u AvgQLen[%04u] minRsv[%02u] CurTcRes[%02u] GrtdTcRes[%02u]\n",
	...
	DBGLOG(SW4, WARN, ("Resource Residual[%u] ExtraRsv[%u]\n",
			   prQM->u4ResidualTcResource, prQM->u4ExtraReservedTcResource));
	DBGLOG(SW4, WARN, ("QueLenMovingAvg[%u] Time2AdjResource[%u] Time2UpdateQLen[%u]\n",
	...
	for (i = 0; i < NUM_OF_PER_STA_TX_QUEUES; i++) {
		DBGLOG(SW4, WARN, ("TC%u HeadSta[%u] ResourceUsedCount[%u]\n",
				   i, prQM->au4HeadStaRecIndex[i], prQM->au4ResourceUsedCount[i]));
	}
#endif

	DBGLOG(SW4, WARN, ("BMC or unknown TxQueue Len[%u]\n", prQM->arTxQueue[0].u4NumElem));
	DBGLOG(SW4, WARN, ("Pending QLen Normal[%u] Sec[%u]\n",
			   prGlueInfo->i4TxPendingFrameNum, prGlueInfo->i4TxPendingSecurityFrameNum));

#if defined(LINUX)
	for (i = 0; i < HW_BSSID_NUM; i++) {
		DBGLOG(SW4, WARN, ("Pending BSS[%u] QLen[%u:%u:%u:%u]\n",
...
#endif
	DBGLOG(SW4, WARN, ("Pending FWD CNT[%d]\n", prTxCtrl->i4PendingFwdFrameCount));
	DBGLOG(SW4, WARN, ("Pending MGMT CNT[%d]\n", prTxCtrl->i4TxMgmtPendingNum));

	DBGLOG(SW4, WARN, ("---------------------------------\n"));

	DBGLOG(SW4, WARN, ("Total RFB[%u]\n", CFG_RX_MAX_PKT_NUM));
	DBGLOG(SW4, WARN, ("rFreeSwRfbList[%u]\n", prAdapter->rRxCtrl.rFreeSwRfbList.u4NumElem));
	DBGLOG(SW4, WARN, ("rReceivedRfbList[%u]\n", prAdapter->rRxCtrl.rReceivedRfbList.u4NumElem));
	DBGLOG(SW4, WARN, ("rIndicatedRfbList[%u]\n", prAdapter->rRxCtrl.rIndicatedRfbList.u4NumElem));
	DBGLOG(SW4, WARN, ("ucNumIndPacket[%u]\n", prAdapter->rRxCtrl.ucNumIndPacket));
	DBGLOG(SW4, WARN, ("ucNumRetainedPacket[%u]\n", prAdapter->rRxCtrl.ucNumRetainedPacket));

	DBGLOG(SW4, WARN, ("---------------------------------\n"));
	DBGLOG(SW4, WARN, ("CMD: FreeCmd[%u/%u] PendingCmd[%u] Cmd2Tx[%u]\n",
	...
	DBGLOG(SW4, WARN, ("MGMT: FreeMgmt[%u/%u] PendingMgmt[%u]\n",
	...
	do {
		...
		if (fgIsNewEntryAdded) {	/* Only when a new BA entry is indeed added will the host be notified */
			...
			if (!prSwRfbEventToHost) {
				...
				RXM_SET_EVENT_PACKET((P_SW_RFB_T) prSwRfbEventToHost,
