...
VOID nicRxFillRadiotapVHT(IN OUT P_MONITOR_RADIOTAP_T prMonitorRadiotap, IN P_HW_MAC_RX_STS_GROUP_3_T prRxStatusGroup3)
{
	...
	prMonitorRadiotap->ucVhtCoding = (((prRxStatusGroup3)->u4RxVector[0] & RX_VT_CODING_MASK) >>
					RX_VT_CODING_OFFSET);	/* VHTA2 B2-B3 */

	prMonitorRadiotap->ucVhtGroupId = (((((prRxStatusGroup3)->u4RxVector[1] & RX_VT_GROUPID_1_MASK) >>
					RX_VT_GROUPID_1_OFFSET) << 2) |
					(((prRxStatusGroup3)->u4RxVector[0] &
					RX_VT_GROUPID_0_MASK) >> RX_VT_GROUPID_0_OFFSET)); /* VHTA1 B4-B9 */
	/* VHTA1 B13-B21 */
	...
	case EVENT_ID_ADD_PKEY_DONE:
		{
			...
			DBGLOG(RSN, TRACE,
			       ...
			       prAddKeyDone->ucBSSIndex, prAddKeyDone->ucKeyId,
...
VOID nicRxProcessMgmtPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (HAL_RX_STATUS_IS_ICV_ERROR(prSwRfb->prRxStatus)) {
		if (HAL_RX_STATUS_GET_SEC_MODE(prSwRfb->prRxStatus) == CIPHER_SUITE_BIP) {
			DBGLOG(RSN, INFO,
				("[MFP] RX with BIP ICV ERROR\n"));
		} else {
			DBGLOG(RSN, INFO,
				("[MFP] RX with ICV ERROR\n"));
		}
		nicRxReturnRFB(prAdapter, prSwRfb);
