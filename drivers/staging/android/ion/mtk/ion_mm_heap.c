				      ...
				      struct ion_buffer *buffer, unsigned long order)
{
	...
	if (!page) {
		pr_info("%s: [ion_dbg] warning: alloc_pages order=%lu cache=%d\n", __func__, order,
		       cached);
		return NULL;
	}
...
static int ion_mm_heap_debug_show(struct ion_heap *heap, struct seq_file *s, void *unused)
{
...

	{
		...
		for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
			...
			ION_PRINT_LOG_OR_SEQ(s, "%pK %8zu %3d %3d %3d %3d %8x %3u %3lu %3d %s "
	...
	for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
		...
		if (client->task) {
			...
			ION_PRINT_LOG_OR_SEQ(s, "client(0x%pK) %s (%s) pid(%u) ================>\n",
					     client, task_comm, client->dbg_name, client->pid);
		} else {
			ION_PRINT_LOG_OR_SEQ(s,
					     "client(0x%pK) %s (from_kernel) pid(%u) ================>\n",
...

		{
			...
			for (m = rb_first(&client->handles); m; m = rb_next(m)) {
				...
				ION_PRINT_LOG_OR_SEQ(s,
						     "\thandle=%pK, buffer=%pK, heap=%d, backtrace is:\n",
						     handle,
...
void ion_mm_heap_memory_detail(void)
{
	...
	for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
		...
		if (client->task) {
			...
			get_task_comm(task_comm, client->task);
			ION_PRINT_LOG_OR_SEQ(NULL, "%16.s(%16.s) %16u %16zu 0x%pK\n", task_comm,
	...
	case ION_MM_CONFIG_BUFFER:
		if (Param.config_buffer_param.handle) {
			...
			if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
				...
				char *errMsg = "Cannot configure buffer that is not from multimedia heap";
				IONMSG
				    ("[ion_mm_heap]: Error. %s.\n", errMsg);
				ret = 0;
			}
	...
	case ION_MM_SET_DEBUG_INFO:
		{
			...
			if (Param.buf_debug_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					char *errMsg = "Cannot set dbg buffer that is not from multimedia heap.";
					IONMSG
					    ("[ion_mm_heap]: Error. %s.\n", errMsg);
					ret = -EFAULT;
				}
	...
	case ION_MM_GET_DEBUG_INFO:
		{
			...
			if (Param.buf_debug_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					char *errMsg = "Cannot get dbg buffer that is not from multimedia heap.";
					IONMSG
					    ("[ion_mm_heap]: Error. %s.\n", errMsg);
					ret = -EFAULT;
				}
	...
	case ION_MM_SET_SF_BUF_INFO:
		{
			...
			if (Param.sf_buf_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					char *errMsg = "Cannot set sf_buf_info buffer that is not from multimedia heap.";
					IONMSG
					    ("[ion_mm_heap]: Error. %s.\n", errMsg);
					ret = -EFAULT;
				}
	...
	case ION_MM_GET_SF_BUF_INFO:
		{
			...
			if (Param.sf_buf_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					char *errMsg = "Cannot get sf_buf_info buffer that is not from multimedia heap.";
					IONMSG
					    ("[ion_mm_heap]: Error. %s.\n", errMsg);
					ret = -EFAULT;
				}
