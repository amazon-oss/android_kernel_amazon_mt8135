...
/****  Dump Stop Events ****/
		...
		e_pmpt->cur_ts == 0 ?
		aee_wdt_printf("[Preempt disable] max duration:%llu ns(s: %llu, e: %llu)\n\n",
		e_pmpt->last_te - e_pmpt->last_ts, e_pmpt->last_ts, e_pmpt->last_te) :
		aee_wdt_printf("[Preempt disable] cur_ts:%llu(elapsed:%llu ns),"
			"max duration:%llu ns(s: %llu, e: %llu)\n\n",
			e_pmpt->cur_ts, sched_clock() - e_pmpt->cur_ts, e_pmpt->last_te - e_pmpt->last_ts,
			e_pmpt->last_ts, e_pmpt->last_te);

		if (e_pmpt->cur_ts) {
			aee_wdt_printf("\n[Preempt disable] is disabled since\n");
			aee_wdt_printf("0x%x ", e_pmpt->cur_start_addr0);
			sprint_symbol(buf, e_pmpt->cur_start_addr0);
			aee_wdt_printf("(%s)\n", buf);
			aee_wdt_printf("0x%x ", e_pmpt->cur_start_addr1);
			sprint_symbol(buf, e_pmpt->cur_start_addr1);
			aee_wdt_printf("(%s)\n", buf);
		}

		if (e_pmpt->start_addr0) {
			aee_wdt_printf("\n[Preempt disable] max duration by\n");
			aee_wdt_printf("0x%x ", e_pmpt->start_addr0);
			sprint_symbol(buf, e_pmpt->start_addr0);
			aee_wdt_printf("(%s)\n", buf);
			aee_wdt_printf("0x%x ", e_pmpt->start_addr1);
			sprint_symbol(buf, e_pmpt->start_addr1);
			aee_wdt_printf("(%s)\n", buf);
			aee_wdt_printf("=================================\n");
			aee_wdt_printf("0x%x ", e_pmpt->end_addr0);
			sprint_symbol(buf, e_pmpt->end_addr0);
			aee_wdt_printf("(%s)\n", buf);
			aee_wdt_printf("0x%x ", e_pmpt->end_addr1);
			sprint_symbol(buf, e_pmpt->end_addr1);
			aee_wdt_printf("(%s)\n", buf);
		}
