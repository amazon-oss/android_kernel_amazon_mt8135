...
/* Preempt off monitor */
void MT_trace_preempt_off(unsigned long addr0, unsigned long addr1)
{
	struct sched_stop_event *e;
	e = &__raw_get_cpu_var(Preempt_disable_mon);

	e->cur_ts = sched_clock();
	e->cur_start_addr0 = addr0;
	e->cur_start_addr1 = addr1;
}

#define TIME ((u64)TIME_1S * 10)

void MT_trace_preempt_on(unsigned long addr0, unsigned long addr1)
{
	struct sched_stop_event *e;
	e = &__raw_get_cpu_var(Preempt_disable_mon);
	...
	if ((sched_clock() - e->cur_ts > e->max_duration) ||
		(sched_clock() - e->last_te > TIME)) {
		e->last_ts = e->cur_ts;
		e->last_te = sched_clock();
		e->max_duration = e->last_te - e->last_ts;
		e->start_addr0 = e->cur_start_addr0;
		e->start_addr1 = e->cur_start_addr1;
		e->end_addr0 = addr0;
		e->end_addr1 = addr1;
	}
...
static int __init init_mtsched_mon(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		per_cpu(mtsched_mon_enabled, cpu) = 0x0;	/* 0x1 || 0x2, IRQ & Preempt */
