...
void hdmi_dump(void)
{
	...
	pr_err("[HDMI_SHELL]--------------HDMI SHELL DUMP-------------------\n");
	for (u4addr = 0; u4addr <= 0x390; u4addr = u4addr+4) {
		internal_hdmi_read(0xF400F800 + u4addr, &u4regval);
		pr_err("0x%x = 0x%x\n", (0xF400F800 + u4addr), u4regval);
	}

	pr_err("[HDMI_CEC]--------------HDMI DPI DUMP-------------------\n");
	for (u4addr = 0; u4addr <= 0xD4; u4addr = u4addr+4) {
		internal_hdmi_read(0xF400F000 + u4addr, &u4regval);
		pr_err("0x%x = 0x%x\n", (0xF400F000 + u4addr), u4regval);
	}
}
