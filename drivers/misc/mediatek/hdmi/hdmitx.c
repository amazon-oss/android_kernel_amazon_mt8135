...
void hdmi_enablecolorbar(bool fgen)
{
	if (fgen)
		hdmi_drv->write(0x00b4, 0x41);
	else
		hdmi_drv->write(0x00b4, 0);
}
...
ion_phys_addr_t get_ion_physical_address(struct ion_handle *ion_handle)
{
	...
	data.config_buffer_param.kernel_handle = ion_handle;
...
static unsigned int hdmi_get_width(HDMI_VIDEO_RESOLUTION r)
{
	...
	ASSERT(r < ARRAY_SIZE(hdmi_resolution_param_table));
