...
#define PK_XLOG_INFO(fmt, args...) printk(PFX "[%s] " fmt, __func__, ##args)
#else
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_CAL_DATA:  /*Chia-Hung Lin for EEPROM*/
					 ...
					 loff_t *ppos)
{
	...
	ssize_t bytes;
	*ppos = (*ppos >= buffer_size) ? buffer_size : *ppos;
	bytes = count < (buffer_size-(*ppos)) ? count : (buffer_size-(*ppos));

	sprintf(buffer, "Read success! 0x%02x 0x%02x\n", g_MainSensorReg.RegAddr, g_MainSensorReg.RegData);

	if (copy_to_user(pPage, buffer , sizeof(char)*bytes))
		return -EFAULT;
...
static ssize_t CAMERA_Temperature_Read(struct file *file, char __user *pPage, size_t count, loff_t *ppos)
{
	...
	char buffer[10] = {0};
	ssize_t bytes;
	*ppos = (*ppos >= 10) ? 10 : *ppos;
	bytes = count < (10-(*ppos)) ? count : (10-(*ppos));
...
static int __init CAMERA_HW_i2C_init(void)
{
	...
	main_sensor_debugfs_create();
	sub_sensor_debugfs_create();
#endif
