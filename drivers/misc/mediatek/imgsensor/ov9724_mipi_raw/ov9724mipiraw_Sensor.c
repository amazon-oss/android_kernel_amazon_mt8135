...
EEPROM_LSC_DATA_T OV9724_lsc_eeprom_data;
int OV9724_eeprom_flag = 0;
...
kal_uint16 OV9724_read_cmos_sensor(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *) &get_byte, 1, OV9724MIPI_WRITE_ID);

	return get_byte;
}

/**my code***/
inline kal_uint16 OV9724_read_eeprom_sensor(kal_uint16 addr)
{
	kal_uint16 get_byte = 0;
	iReadReg((u16) addr, (u8 *) &get_byte, OV9724_EEPROM_ID);
	return get_byte;
}

/**my code***/

static void OV9724_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
	char pu_send_cmd[3] = { (char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF) };
	iWriteRegI2C(pu_send_cmd, 3, OV9724MIPI_WRITE_ID);
}
...
kal_uint32 OV9724_CalcExtra_For_ShutterMargin(kal_uint32 shutter_value,
...
kal_uint32 OV9724_CalcFrameLength_For_AutoFlicker(void)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		AutoFlicker_min_framelength =
		    (OV9724.capPclk * 10000) / (OV9724_FULL_PERIOD_PIXEL_NUMS +
						OV9724.DummyPixels) / OV9724_AUTOFLICKER_OFFSET_30 *
		    10;
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		if (OV9724_VIDEO_MODE_TARGET_FPS == 30) {
			AutoFlicker_min_framelength =
			    (OV9724.videoPclk * 10000) / (OV9724_VIDEO_PERIOD_PIXEL_NUMS +
							  OV9724.DummyPixels) /
			    OV9724_AUTOFLICKER_OFFSET_30 * 10;
		} else if (OV9724_VIDEO_MODE_TARGET_FPS == 15) {
			AutoFlicker_min_framelength =
			    (OV9724.videoPclk * 10000) / (OV9724_VIDEO_PERIOD_PIXEL_NUMS +
							  OV9724.DummyPixels) /
			    OV9724_AUTOFLICKER_OFFSET_15 * 10;
		} else {
			AutoFlicker_min_framelength =
			    OV9724_VIDEO_PERIOD_LINE_NUMS + OV9724.DummyLines;
		}
		break;
	...
	default:
		AutoFlicker_min_framelength =
		    (OV9724.pvPclk * 10000) / (OV9724_PV_PERIOD_PIXEL_NUMS +
					       OV9724.DummyPixels) / OV9724_AUTOFLICKER_OFFSET_30 *
		    10;
		break;
	}
...
void OV9724_write_shutter(kal_uint32 shutter)
{
	...
	if (sub_sensor_init_setting_switch == 5)
		return; /* manaul exposure gain mode */
	...
	if (sub_sensor_init_setting_switch == 0) {
		...
		frame_length =
		    frame_length + OV9724_CalcExtra_For_ShutterMargin(shutter, max_shutter);
...
void OV9724_SetGain(UINT16 iGain)
{
	...
	default:
		...
		spin_unlock_irqrestore(&OV9724mipiraw_drv_lock, flags);
		reg_gain = OV9724.sensorGlobalGain;
		break;
	}

	write_OV9724_gain(reg_gain);
	OV9724DB("[OV9724_SetGain]:FATP mode (%d), BB_setValue =%d,Sensor_reg_value=0x%x\n",
		 sub_sensor_init_setting_switch, OV9724.realGain, OV9724.sensorGlobalGain);
...
void OV9724_get_sensor_group_info(kal_uint16 group_idx, kal_int8 *group_name_ptr,
...
void OV9724_get_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx,
				 MSDK_SENSOR_ITEM_INFO_STRUCT *info_ptr)
{
	kal_int16 temp_reg = 0;
	kal_uint16 temp_gain = 0, temp_addr = 0, temp_para = 0;

	OV9724DB("[OV9725] %s():%d group_idx=%d\n", __func__, __LINE__, group_idx);
	switch (group_idx) {
	case PRE_GAIN:
		switch (item_idx) {
		case 0:
...
static void OV9724_Sensor_Init(void)
{
	...
	OV9724_write_cmos_sensor(0x0203, 0xf9);	/* exposure low bit */

	OV9724_write_cmos_sensor(0x4800, 0x64);/* MIPI Clock Gating ON */
	...
	OV9724_write_cmos_sensor(0x4819, 0x99);/* mipi THS ZERO MIN */
	OV9724_write_cmos_sensor(0x4827, 0x34);/* mipi THS PREPARE MIN */

	OV9724_write_cmos_sensor(0x0101, 0x02);/* flip */


	OV9724_write_cmos_sensor(0x5110, 0x09);/*  */
	OV9724_write_cmos_sensor(0x4307, 0x3a);/*  */
	OV9724_write_cmos_sensor(0x5000, 0x06);/* BPC on;  on:0x06 off:0x00 */
	...
	if (sub_sensor_size != 0) {
		pr_err("OV9724: send additonal debugfs based settings to the sensor size[%d]\n", sub_sensor_size);
		for (i = 0; i < sub_sensor_size; i++) {
			pr_err("OV9724: Addr[%x] Val[%x]\n", sub_sensor_dynamic_script[i].addr,
			sub_sensor_dynamic_script[i].value);
			OV9724_write_cmos_sensor(sub_sensor_dynamic_script[i].addr, sub_sensor_dynamic_script[i].value);
		}
...
UINT32 OV9724Open(void)
{
	...
	for (i = 0; i < 2; i++) {
		sensor_id =
		    (OV9724_read_cmos_sensor(0x300A) << 8) | OV9724_read_cmos_sensor(0x300B);
...
UINT32 OV9724GetSensorID(UINT32 *sensorID)
{
	...
	MYLOG("OV9724 get lsc EEPROM data on first open\n");
	OV9724_get_eeprom_lsc_data(&OV9724_lsc_eeprom_data, &OV9724_eeprom_flag);
	OV9724_eeprom_flag = 1;
	MYLOG("OV9724 flag %d\n", OV9724_eeprom_flag);
	MYLOG("OV9724 get lsc EEPROM data Done!\n");
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	if (SENSOR_MODE_CAPTURE == OV9724.sensorMode) {
		...
		OV9724.DummyPixels = 0;
		OV9724.DummyLines = 0;
		OV9724_FeatureControl_PERIOD_PixelNum =
		    OV9724_FULL_PERIOD_PIXEL_NUMS + OV9724.DummyPixels;
		OV9724_FeatureControl_PERIOD_LineNum =
		    OV9724_FULL_PERIOD_LINE_NUMS + OV9724.DummyLines;
		spin_unlock(&OV9724mipiraw_drv_lock);
...
UINT32 OV9724Control(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
kal_uint32 OV9724_SET_FrameLength_ByVideoMode(UINT16 Video_TargetFps)
{
	...
	if (OV9724.OV9724AutoFlickerMode == KAL_TRUE) {
		...
		MIN_FrameLength =
		    (OV9724.videoPclk * 10000) / (OV9724_VIDEO_PERIOD_PIXEL_NUMS +
						  OV9724.DummyPixels) / frameRate * 10;
	} else
		MIN_FrameLength =
		    (OV9724.videoPclk * 10000) / (OV9724_VIDEO_PERIOD_PIXEL_NUMS +
						  OV9724.DummyPixels) / Video_TargetFps;
...
UINT32 OV9724SetVideoMode(UINT16 u2FrameRate)
{
	...
	/* if (OV9724.sensorMode == SENSOR_MODE_VIDEO) { */
		...
		if ((MIN_Frame_length <= OV9724_VIDEO_PERIOD_LINE_NUMS)) {
			...
			OV9724DB
			    ("[OV9724SetVideoMode]config LINE_NUMS < Org_setting ,please check it.\n");
		}
		OV9724DB("[OV9724SetVideoMode]current fps (10 base)= %d\n",
			 (OV9724.videoPclk * 10000) * 10 / (OV9724_VIDEO_PERIOD_PIXEL_NUMS +
							    OV9724.DummyPixels) / MIN_Frame_length);
...
UINT32 OV9724MIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId,
					       MUINT32 *pframeRate)
{
			    ...
			    UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
		if (*pFeatureParaLen >= sizeof(NVRAM_SENSOR_DATA_STRUCT)) {
			...
			pSensorDefaultData->SensorId = OV9724MIPI_SENSOR_ID;
			memcpy(pSensorDefaultData->SensorEngReg, OV9724SensorReg,
			       sizeof(SENSOR_REG_STRUCT) * ENGINEER_END);
			memcpy(pSensorDefaultData->SensorCCTReg, OV9724SensorCCT,
	...
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		memcpy(pSensorConfigData, &OV9724SensorConfigData,
	...
	case SENSOR_FEATURE_GET_GROUP_INFO:
		OV9724_get_sensor_group_info(pSensorGroupInfo->GroupIdx,
					     pSensorGroupInfo->GroupNamePtr,
					     &pSensorGroupInfo->ItemCount);
		*pFeatureParaLen = sizeof(MSDK_SENSOR_GROUP_INFO_STRUCT);
		break;
	case SENSOR_FEATURE_GET_ITEM_INFO:
		OV9724_get_sensor_item_info(pSensorItemInfo->GroupIdx, pSensorItemInfo->ItemIdx,
					    pSensorItemInfo);
		*pFeatureParaLen = sizeof(MSDK_SENSOR_ITEM_INFO_STRUCT);
		break;

	case SENSOR_FEATURE_SET_ITEM_INFO:
		OV9724_set_sensor_item_info(pSensorItemInfo->GroupIdx, pSensorItemInfo->ItemIdx,
					    pSensorItemInfo->ItemValue);
		*pFeatureParaLen = sizeof(MSDK_SENSOR_ITEM_INFO_STRUCT);
		break;

	case SENSOR_FEATURE_GET_ENG_INFO:
		pSensorEngInfo->SensorId = OV9724MIPI_SENSOR_ID;
	...
	case SENSOR_FEATURE_GET_FACTORY_MODE:
		*pFeatureReturnPara32 = sub_sensor_init_setting_switch;
		*pFeatureParaLen = 4;
		OV9724DB("[%s] == SENSOR_FEATURE_GET_FACTORY_MODE == Factory mode = %d\n",
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		OV9724MIPISetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
						    *(pFeatureData32 + 1));
		break;
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		OV9724MIPIGetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
	...
	case SENSOR_FEATURE_GET_EEPROM_CAL_DATA:	/*Chia-hung Lin read EEPROM */
		...
		/* MYLOG("OV9724 test size: %d,  %d\n", sizeof(pFeaturePara), sizeof(*pFeaturePara) ); */
		/* MYLOG("OV9724 address: 0x%x,  0x%x\n", &pFeaturePara[0], &pFeaturePara[8191] ); */
		OV9724_get_eeprom_full_data(pSensorEepromInfo);
		*pFeatureParaLen = sizeof(EEPROM_ALL_DATA_T);
		MYLOG("OV9724 EEPROM meta data size: %d\n", sizeof(*pSensorEepromInfo));
		/* MYLOG("OV9724 EEPROM meta data: Mode: 0x%02x,
		Year: 0x%02x\n", pFeaturePara[32], pFeaturePara[42] ); */
		break;
	case SENSOR_FEATURE_GET_EEPROM_AWB_DATA:	/*Chia-hung Lin read EEPROM */
		OV9724_get_eeprom_awb_data(pSensorEepromAWBInfo);
		*pFeatureParaLen = sizeof(EEPROM_AWB_DATA_T);
		MYLOG("OV9724 EEPROM meta data size: %d\n", sizeof(*pSensorEepromAWBInfo));
		break;
	case SENSOR_FEATURE_GET_EEPROM_LSC_DATA:	/*Chia-hung Lin read EEPROM */
		OV9724_get_eeprom_lsc_data(pSensorEepromLSCInfo, &OV9724_eeprom_flag);
