...
#define SHUTTER_FRAMELENGTH_MARGIN 18
...
EEPROM_LSC_DATA_T S5K5E2YA_lsc_eeprom_data;
int S5K5E2YA_eeprom_flag = 0;
...
/**my code***/
kal_uint16 S5K5E2YA_read_eeprom_sensor(kal_uint16 addr)
{
	kal_uint16 get_byte = 0;
	iReadReg((u16) addr, (u8 *) &get_byte, S5K5E2YAMIPI_EEPROM_ID);
	return get_byte;
}
...
void S5K5E2YA_wordwrite_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
	char puSendCmd[4] = { (char)(addr >> 8), (char)(addr & 0xFF), (char)(para >> 8), (char)(para & 0xFF) };
	iWriteRegI2C(puSendCmd, 4, S5K5E2YAMIPI_WRITE_ID);
}

void S5K5E2YA_bytewrite_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
...
void S5K5E2YA_MIPI_read_otp_wb(struct S5K5E2YA_MIPI_otp_struct *otp)
{
	...
	for (PageCount = 4; PageCount >= 1; PageCount--) {
		...
		R_to_G =
		    (S5K5E2YA_read_cmos_sensor(0x3a09) << 8) + S5K5E2YA_read_cmos_sensor(0x3a0a);
		B_to_G =
		    (S5K5E2YA_read_cmos_sensor(0x3a0b) << 8) + S5K5E2YA_read_cmos_sensor(0x3a0c);
		G_to_G =
		    (S5K5E2YA_read_cmos_sensor(0x3a0d) << 8) + S5K5E2YA_read_cmos_sensor(0x3a0e);
...
static inline kal_uint32 GetScenarioPixelClock(void)
{
	...
	SENSORDB("enter getscenariopixelclock %d pclk=%d\n", s_S5K5E2YACurrentScenarioId, pclk);
	switch (s_S5K5E2YACurrentScenarioId) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
...
{
	if (main_sensor_init_setting_switch == 5)
		return;/* manual exposure gain mode */
...
void S5K5E2YA_write_shutter(kal_uint32 shutter)
{
	...
	if (main_sensor_init_setting_switch == 0) {
		...
		SENSORDB("enter s5k5e2 write shutter %d", shutter);
		...
		S5K5E2YA_bytewrite_cmos_sensor(0x0340, 0x08);	/* frame_length_lines : def. 1962d */
		S5K5E2YA_bytewrite_cmos_sensor(0x0341, 0x40);

		/* Integration time */
		S5K5E2YA_bytewrite_cmos_sensor(0x0202, 0x07);	/*  coarse integration */
		S5K5E2YA_bytewrite_cmos_sensor(0x0203, 0xF0);
	}
...
void S5K5E2YA_SetGain(UINT16 gain)
{
	...
	spin_lock_irqsave(&s5k5e2yamipiraw_drv_lock, flags);
	s5k5e2ya.sensorGain = reg_gain;
	spin_unlock_irqrestore(&s5k5e2yamipiraw_drv_lock, flags);

	/* S5K5E2YA_bytewrite_cmos_sensor(0x0104, 0x01); *//* Grouped parameter hold */
	S5K5E2YA_wordwrite_cmos_sensor(0x0204, reg_gain);	/* sensor base gain is 32 */
	/* S5K5E2YA_bytewrite_cmos_sensor(0x0104, 0x00); *//* Grouped parameter release */

	SENSORDB("reg_gain 0x%04x", reg_gain);
}
...
void S5K5E2YA_get_sensor_group_info(kal_uint16 group_idx, kal_int8 *group_name_ptr,
...
void S5K5E2YA_get_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx,
				   MSDK_SENSOR_ITEM_INFO_STRUCT *info_ptr)
{
	kal_int16 temp_reg = 0;
	SENSORDB("[S5K5E2YA] %s():%d group_idx=%d\n", __func__, __LINE__, group_idx);
	switch (group_idx) {
	case CMMCLK_CURRENT:
		switch (item_idx) {
		case 0:
...
kal_bool S5K5E2YA_set_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx,
				       kal_int32 ItemValue)
{
/*
...
void S5K5E2YAPreviewSetting(void)
{
	...
	/*  Clock Setting */
	S5K5E2YA_bytewrite_cmos_sensor(0x0305, 0x05);	/* PLLP (def:5) */
	S5K5E2YA_bytewrite_cmos_sensor(0x0306, 0x00);
	S5K5E2YA_bytewrite_cmos_sensor(0x0307, 0xC3);	/* PLLM (195 * (24/5) = 936) */
	S5K5E2YA_bytewrite_cmos_sensor(0x3C1F, 0x00);	/* PLLS  */

	S5K5E2YA_bytewrite_cmos_sensor(0x0820, 0x03);	/* requested link bit rate mbps : (936Mbps) */
	S5K5E2YA_bytewrite_cmos_sensor(0x0821, 0xA8);
	S5K5E2YA_bytewrite_cmos_sensor(0x3C1C, 0x59);	/* dbr_div */

	/* MIPI setting for MCLK 24MHz */
	S5K5E2YA_bytewrite_cmos_sensor(0x30BC, 0x38);
	S5K5E2YA_bytewrite_cmos_sensor(0x30BD, 0x40);
	S5K5E2YA_bytewrite_cmos_sensor(0x3110, 0x70);
	S5K5E2YA_bytewrite_cmos_sensor(0x3111, 0x80);
	S5K5E2YA_bytewrite_cmos_sensor(0x3112, 0x7B);
	S5K5E2YA_bytewrite_cmos_sensor(0x3113, 0xC0);
	S5K5E2YA_bytewrite_cmos_sensor(0x30C7, 0x1A);

	/*  Size Setting */
	S5K5E2YA_bytewrite_cmos_sensor(0x0340, 0x08);	/* frame_length_lines : def. 2112d (30fps) */
	S5K5E2YA_bytewrite_cmos_sensor(0x0341, 0x40);
	...
	/* Integration time */
	S5K5E2YA_bytewrite_cmos_sensor(0x0202, 0x07);	/* coarse integration (EIT=32ms) */
	S5K5E2YA_bytewrite_cmos_sensor(0x0203, 0xF0);
	S5K5E2YA_bytewrite_cmos_sensor(0x0200, 0x04);	/* fine integration */
	S5K5E2YA_bytewrite_cmos_sensor(0x0201, 0x98);
	...
	S5K5E2YA_bytewrite_cmos_sensor(0x3008, 0x55);	/*  ct_tx_width */
	S5K5E2YA_bytewrite_cmos_sensor(0x3039, 0x00);	/*  cintc1_margin */
	S5K5E2YA_bytewrite_cmos_sensor(0x303A, 0x00);	/*  cintc2_margin */
	S5K5E2YA_bytewrite_cmos_sensor(0x303B, 0x00);	/*  offs_sh */
	S5K5E2YA_bytewrite_cmos_sensor(0x3009, 0x05);	/*  ct_srx_margin */
	S5K5E2YA_bytewrite_cmos_sensor(0x300A, 0x55);	/*  ct_stx_width */
	S5K5E2YA_bytewrite_cmos_sensor(0x300B, 0x38);	/*  ct_dstx_width */
	...
	S5K5E2YA_bytewrite_cmos_sensor(0x3014, 0x22);	/*  ct_s1s_end */
	S5K5E2YA_bytewrite_cmos_sensor(0x300E, 0x79);	/*  ct_S5K5E2YA_bytewrite_cmos_sensor(0x3_width */
	S5K5E2YA_bytewrite_cmos_sensor(0x3010, 0x68);	/*  ct_s4_width */
	S5K5E2YA_bytewrite_cmos_sensor(0x3019, 0x03);	/*  ct_s4d_start */
	S5K5E2YA_bytewrite_cmos_sensor(0x301A, 0x00);	/*  ct_pbr_start */
	S5K5E2YA_bytewrite_cmos_sensor(0x301B, 0x06);	/*  ct_pbr_width */
	S5K5E2YA_bytewrite_cmos_sensor(0x301C, 0x00);	/*  ct_pbs_start */
	S5K5E2YA_bytewrite_cmos_sensor(0x301D, 0x22);	/*  ct_pbs_width */
	S5K5E2YA_bytewrite_cmos_sensor(0x301E, 0x00);	/*  ct_pbr_ob_start */
	S5K5E2YA_bytewrite_cmos_sensor(0x301F, 0x10);	/*  ct_pbr_ob_width */
	...
	S5K5E2YA_bytewrite_cmos_sensor(0x3023, 0x1E);	/*  ct_crs_start */
	...
	S5K5E2YA_bytewrite_cmos_sensor(0x3028, 0x1A);	/*  ct_rmp_off_start */
	S5K5E2YA_bytewrite_cmos_sensor(0x3015, 0x00);	/*  ct_rmp_rst_start (MSB) */
	S5K5E2YA_bytewrite_cmos_sensor(0x3016, 0x84);	/*  ct_rmp_rst_start (LSB) */
	S5K5E2YA_bytewrite_cmos_sensor(0x3017, 0x00);	/*  ct_rmp_sig_start (MSB) */
	S5K5E2YA_bytewrite_cmos_sensor(0x3018, 0xA0);	/*  ct_rmp_sig_start (LSB) */
	...
	/* CDS */
	S5K5E2YA_bytewrite_cmos_sensor(0x3099, 0x0E);	/*  cds_option ([3]:crs switch disable */
...
/*	S5K5E2YA_bytewrite_cmos_sensor(0x320D, 0x00); */
	...
	S5K5E2YA_bytewrite_cmos_sensor(0x3300, 0x0C); /* BPC On */ /* 0C for DPC ON, 0D for DPC OFF */
	...
	/* OB Level */
	S5K5E2YA_bytewrite_cmos_sensor(0x3232, 0x10);
	S5K5E2YA_bytewrite_cmos_sensor(0x3410, 0x10);
	S5K5E2YA_bytewrite_cmos_sensor(0x3C11, 0x10);
...
#ifdef CONFIG_SENSOR_DEBUGFS
	/* send additional debugfs based settings */
	if (main_sensor_size != 0) {
		pr_err("S5K5E2YA: send additonal debugfs based settings to the sensor size[%d]\n", main_sensor_size);
		for (i = 0; i < main_sensor_size; i++) {
			pr_err("S5K5E2YA: Addr[%x] Val[%x]\n", main_sensor_dynamic_script[i].addr,
				main_sensor_dynamic_script[i].value);
			S5K5E2YA_bytewrite_cmos_sensor(main_sensor_dynamic_script[i].addr,
				main_sensor_dynamic_script[i].value);
		}
...
void S5K5E2YAVideoSetting(void)
{
	...
	/* requested link bit rate mbps : (def:3D3h 979d --> 35Bh 859d) */
	S5K5E2YA_bytewrite_cmos_sensor(0x0820, 0x03);
	S5K5E2YA_bytewrite_cmos_sensor(0x0821, 0x80);
	S5K5E2YA_bytewrite_cmos_sensor(0x3C1C, 0x58);	/* dbr_div */

	/*  Size Setting */
	S5K5E2YA_bytewrite_cmos_sensor(0x0340, 0x07);	/* frame_length_lines : def. 1962d (7C2 --> 7A6 Min 23 lines) */
	...
	S5K5E2YA_bytewrite_cmos_sensor(0x3233, 0x00);	/*  D-pedestal L/F ADLC off (1FC0h) */
	S5K5E2YA_bytewrite_cmos_sensor(0x3234, 0x00);
	S5K5E2YA_bytewrite_cmos_sensor(0x3300, 0x0D);	/* BPC Off 0x0C for ON */
	S5K5E2YA_bytewrite_cmos_sensor(0x0204, 0x00);	/* Analog gain x1 */
	S5K5E2YA_bytewrite_cmos_sensor(0x0205, 0x20);
...
void S5K5E2YACaptureSetting(void)
{
	...
	S5K5E2YA_bytewrite_cmos_sensor(0x3233, 0x00);	/*  D-pedestal L/F ADLC off (1FC0h) */
	S5K5E2YA_bytewrite_cmos_sensor(0x3234, 0x00);
	S5K5E2YA_bytewrite_cmos_sensor(0x3300, 0x0D);	/* BPC Off, 0C for DPC ON, 0D for DPC OFF */
...
static void S5K5E2YA_Sensor_Init(void)
{
	S5K5E2YA_bytewrite_cmos_sensor(0x3929, 0x07); /* MIPI Clock Gating */
	S5K5E2YA_bytewrite_cmos_sensor(0x3931, 0x02); /* MIPI Output Voltage Settings */
...
UINT32 S5K5E2YAOpen(void)
{
	...
	if (S5K5E2YAMIPI_SENSOR_ID != sensor_id)
...
UINT32 S5K5E2YAGetSensorID(UINT32 *sensorID)
{
	...
	*sensorID = (S5K5E2YA_read_cmos_sensor(0x0000) << 8) | (S5K5E2YA_read_cmos_sensor(0x0001));
	SENSORDB("Read sensor ID=0x%x\n", *sensorID);
	if (*sensorID != S5K5E2YAMIPI_SENSOR_ID) {
		*sensorID = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}

	MYLOG("S5K5E2YA get lsc EEPROM data on first open\n");
	S5K5E2YA_get_eeprom_lsc_data(&S5K5E2YA_lsc_eeprom_data, &S5K5E2YA_eeprom_flag);
	S5K5E2YA_eeprom_flag = 1;
	MYLOG("S5K5E2YA flag %d\n", S5K5E2YA_eeprom_flag);
	MYLOG("S5K5E2YA get lsc EEPROM data Done!\n");
...
{
	/*  smiaRegs_rw_integration_time_coarse_integration_time  */
	return (S5K5E2YA_read_cmos_sensor(0x0202) << 8) | S5K5E2YA_read_cmos_sensor(0x0203);
}
		     ...
		     MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	/* use same setting as preview in video mode */
...
UINT32 S5K5E2YAControl(MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 S5K5E2YAMIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		frameLength =
		    (frameLength >
		     S5K5E2YA_PV_PERIOD_LINE_NUMS) ? (frameLength) : (S5K5E2YA_PV_PERIOD_LINE_NUMS);
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		frameLength =
		    (s5k5e2ya.m_vidPclk) / frameRate * 10 / S5K5E2YA_VIDEO_PERIOD_PIXEL_NUMS;
		frameLength =
		    (frameLength >
		     S5K5E2YA_VIDEO_PERIOD_LINE_NUMS) ? (frameLength)
		    : (S5K5E2YA_VIDEO_PERIOD_LINE_NUMS);
		break;
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		frameLength =
		    (s5k5e2ya.m_vidPclk) / frameRate * 10 / S5K5E2YA_FULL_PERIOD_PIXEL_NUMS;
		frameLength =
		    (frameLength >
		     S5K5E2YA_FULL_PERIOD_LINE_NUMS) ? (frameLength)
		    : (S5K5E2YA_FULL_PERIOD_LINE_NUMS);
		break;
	case MSDK_SCENARIO_ID_CAMERA_ZSD:
		frameLength =
		    (s5k5e2ya.m_vidPclk) / frameRate * 10 / S5K5E2YA_ZSD_PERIOD_PIXEL_NUMS;
		frameLength =
		    (frameLength >
		     S5K5E2YA_ZSD_PERIOD_LINE_NUMS) ? (frameLength)
		    : (S5K5E2YA_ZSD_PERIOD_LINE_NUMS);
		break;
	...
	spin_lock(&s5k5e2yamipiraw_drv_lock);
	shutter = s5k5e2ya.shutter; /* get current shutter value */
#ifdef VARIABLE_FRAME_RATE
	frameLength = (s5k5e2ya.maxExposureLines  >
		     frameLength) ? s5k5e2ya.maxExposureLines : frameLength;
#else /* fixed frame rate */
	if ((shutter + SHUTTER_FRAMELENGTH_MARGIN) >= frameLength) {
		/* reduce shutter to match frame rate */
		shutter = frameLength - SHUTTER_FRAMELENGTH_MARGIN;
		s5k5e2ya.maxExposureLines = frameLength;
		s5k5e2ya.shutter = shutter;
	}
#endif
...
static void S5K5E2YASetMaxFrameRate(UINT16 u2FrameRate)
{
	...
	if (SENSOR_MODE_PREVIEW == s5k5e2ya.sensorMode) {
		...
		FrameHeight =
		    (FrameHeight >
		     S5K5E2YA_PV_PERIOD_LINE_NUMS) ? FrameHeight : S5K5E2YA_PV_PERIOD_LINE_NUMS;
	} else if (SENSOR_MODE_CAPTURE == s5k5e2ya.sensorMode
		   || SENSOR_MODE_ZSD_PREVIEW == s5k5e2ya.sensorMode) {
		FrameHeight =
		    (10 * s5k5e2ya.capPclk) / u2FrameRate / S5K5E2YA_FULL_PERIOD_PIXEL_NUMS;
		FrameHeight =
		    (FrameHeight >
		     S5K5E2YA_FULL_PERIOD_LINE_NUMS) ? FrameHeight : S5K5E2YA_FULL_PERIOD_LINE_NUMS;
	} else {
		FrameHeight =
		    (10 * s5k5e2ya.m_vidPclk) / u2FrameRate / S5K5E2YA_VIDEO_PERIOD_PIXEL_NUMS;
		FrameHeight =
		    (FrameHeight >
		     S5K5E2YA_VIDEO_PERIOD_LINE_NUMS) ? FrameHeight :
		    S5K5E2YA_VIDEO_PERIOD_LINE_NUMS;
	}
...
UINT32 S5K5E2YAMIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId,
						 MUINT32 *pframeRate)
{
			      ...
			      UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	EEPROM_ALL_DATA_T *pSensorEepromInfo = (EEPROM_ALL_DATA_T *) pFeaturePara;
	EEPROM_AWB_DATA_T *pSensorEepromAWBInfo = (EEPROM_AWB_DATA_T *) pFeaturePara;
	EEPROM_LSC_DATA_T *pSensorEepromLSCInfo = (EEPROM_LSC_DATA_T *) pFeaturePara;
	...
	case SENSOR_FEATURE_SET_REGISTER:
		S5K5E2YA_bytewrite_cmos_sensor(pSensorRegData->RegAddr, pSensorRegData->RegData);
		SENSORDB("write addr = 0x%08x, data = 0x%08x\n", pSensorRegData->RegAddr,
			 pSensorRegData->RegData);
		break;
	case SENSOR_FEATURE_GET_REGISTER:
		pSensorRegData->RegData = S5K5E2YA_read_cmos_sensor(pSensorRegData->RegAddr);
		SENSORDB("read addr = 0x%08x, data = 0x%08x\n", pSensorRegData->RegAddr,
			 pSensorRegData->RegData);
		break;
	...
	case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
		if (*pFeatureParaLen >= sizeof(NVRAM_SENSOR_DATA_STRUCT)) {
			...
			pSensorDefaultData->SensorId = S5K5E2YAMIPI_SENSOR_ID;
			memcpy(pSensorDefaultData->SensorEngReg, S5K5E2YASensorReg,
			       sizeof(SENSOR_REG_STRUCT) * ENGINEER_END);
			memcpy(pSensorDefaultData->SensorCCTReg, S5K5E2YASensorCCT,
	...
	case SENSOR_FEATURE_GET_CONFIG_PARA:
		memcpy(pSensorConfigData, &S5K5E2YASensorConfigData,
	...
	case SENSOR_FEATURE_GET_GROUP_INFO:
		S5K5E2YA_get_sensor_group_info(pSensorGroupInfo->GroupIdx,
					       pSensorGroupInfo->GroupNamePtr,
					       &pSensorGroupInfo->ItemCount);
		*pFeatureParaLen = sizeof(MSDK_SENSOR_GROUP_INFO_STRUCT);
		break;
	case SENSOR_FEATURE_GET_ITEM_INFO:
		S5K5E2YA_get_sensor_item_info(pSensorItemInfo->GroupIdx, pSensorItemInfo->ItemIdx,
					      pSensorItemInfo);
		*pFeatureParaLen = sizeof(MSDK_SENSOR_ITEM_INFO_STRUCT);
		break;

	case SENSOR_FEATURE_SET_ITEM_INFO:
		S5K5E2YA_set_sensor_item_info(pSensorItemInfo->GroupIdx, pSensorItemInfo->ItemIdx,
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		S5K5E2YAMIPISetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
						      *(pFeatureData32 + 1));
		break;
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		S5K5E2YAMIPIGetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM) *pFeatureData32,
							  (MUINT32 *) (*(pFeatureData32 + 1)));
		break;
	case SENSOR_FEATURE_GET_EEPROM_CAL_DATA:	/*Chia-hung Lin read EEPROM */
		/* MYLOG("Get data from EEPROM\n"); */
		/* MYLOG("S5K5E2YA test size: %d,  %d\n", sizeof(pFeaturePara), sizeof(*pFeaturePara) ); */
		/* MYLOG("S5K5E2YA address: 0x%x,  0x%x\n", &pFeaturePara[0], &pFeaturePara[8191] ); */
		S5K5E2YA_get_eeprom_full_data(pSensorEepromInfo);
		*pFeatureParaLen = sizeof(EEPROM_ALL_DATA_T);
		MYLOG("S5K5E2YA EEPROM meta data size: %d\n", sizeof(*pSensorEepromInfo));
		/* MYLOG("S5K5E2YA EEPROM meta data:
		Mode: 0x%02x, Year: 0x%02x\n", pFeaturePara[32], pFeaturePara[42] ); */
		break;
	case SENSOR_FEATURE_GET_EEPROM_AWB_DATA:
		S5K5E2YA_get_eeprom_awb_data(pSensorEepromAWBInfo);
		*pFeatureParaLen = sizeof(EEPROM_AWB_DATA_T);
		MYLOG("S5K5E2YA EEPROM meta data size: %d\n", sizeof(*pSensorEepromAWBInfo));
		break;
	case SENSOR_FEATURE_GET_EEPROM_LSC_DATA:
		S5K5E2YA_get_eeprom_lsc_data(pSensorEepromLSCInfo, &S5K5E2YA_eeprom_flag);
		*pFeatureParaLen = sizeof(EEPROM_LSC_DATA_T);
		MYLOG("S5K5E2YA EEPROM meta data size: %d\n", sizeof(*pSensorEepromLSCInfo));
		break;
	default:
		break;
	}
...
UINT32 S5K5E2YAMIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	SENSORDB("%s():%d\n", __func__, __LINE__);
