...
static long MTK_M4U_ioctl(struct file *a_pstFile, unsigned int a_Command, unsigned long a_Param)
{
	...
	case MTK_M4U_T_CONFIG_PORT:
		...
		if ((m4u_port.ePortID >= M4U_PORT_NUM && m4u_port.ePortID != M4U_PORT_UNKNOWN)
		    || m4u_port.ePortID < 0) {
			M4UERR("ePortID=%d, Virtuality=%d, Security=%d, Distance=%d, Direction=%d\n",
...
static unsigned int m4u_do_mva_alloc_fix(unsigned int mva, unsigned int size, void *priv)
{
	...
	if (endIdx > MVA_MAX_BLOCK_NR || region_end > MVA_MAX_BLOCK_NR) {
		M4UMSG("region end error %d %d\n", endIdx, region_end);
		goto out;
	}
...
static int MTK_M4U_isr(M4U_ISR_INFO *pIsrInfo)
{
	...
	pr_err("MTK_M4U_isr-pr clk %d %d\n",
		clock_is_on(MT_CG_INFRA_M4U_PDN), clock_is_on(MT_CG_INFRA_SMI_PDN));
	mt_irq_dump_status(138);
	mt_irq_dump_status(137);
...
int m4u_debug_command(unsigned int command)
{
	...
	switch (command) {
	case 22:
		pr_err("MTK_M4U_isr-pr -irq 0xf020120 0x%x\n",
			readl_relaxed((void *)0xf0200040));
	break;
	case 20:
		pr_err("MTK_M4U_isr-pr -irq 0xf0211210 0x%x\n",
			readl_relaxed((void *)0xf0211210));
	break;
	case 21:
		mt_irq_dump_status(138);
	break;
