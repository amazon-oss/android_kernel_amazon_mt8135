			...
			while (bLockedHW == VAL_FALSE) {
				...
				if (grVcodecEncHWLock.pvHandle == 0)	/* No process use HW, so current process can use HW */
				{
					if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC ||
						...
						bLockedHW = VAL_TRUE;
						venc_power_on();
#ifdef CONFIG_MTK_SEC_WFD_VIDEO_PATH_SUPPORT
	if (rHWLock.bSecureInst == VAL_FALSE) {
		if (request_irq
				(MT_VENC_IRQ_ID , (irq_handler_t)video_intr_dlr2,
				IRQF_TRIGGER_LOW, VCODEC_DEVNAME,
				NULL) < 0) {
			MFV_LOGE
				("[MFV_DEBUG][ERROR] error to request enc irq\n");
		} else {
			MFV_LOGE
				("[MFV_DEBUG] success to request enc irq\n");
		}
	}
#else
...
#ifdef CONFIG_MTK_SEC_WFD_VIDEO_PATH_SUPPORT
	MFV_LOGE("Skip request MT_VENC_IRQ_ID irq in normal world\n");
#else
	if (request_irq
