 ...
 */
struct mtk_logger_reader {
	struct mtk_logger_log *log;	/* associated log */
 ...
 */
static inline struct mtk_logger_log *file_get_log(struct file *file)
{
	if (file->f_mode & FMODE_READ) {
		struct mtk_logger_reader *reader = file->private_data;
		return reader->log;
	} else
 ...
 */
static __u32 get_entry_len(struct mtk_logger_log *log, size_t off)
{
	__u16 val;
 ...
 */
static ssize_t do_read_log_to_user(struct mtk_logger_log *log,
				   struct mtk_logger_reader *reader, char __user *buf, size_t count)
{
	size_t len;
...
static ssize_t warn_overflow(struct mtk_logger_log *log, struct mtk_logger_reader *reader,
			     char __user *buf)
{
	int i;
...
{
	struct mtk_logger_reader *reader = file->private_data;
	struct mtk_logger_log *log = reader->log;
	ssize_t ret;
	unsigned long flags;
 ...
 * Caller must hold log->lock.
 */
static size_t get_next_entry(struct mtk_logger_log *log, size_t off, size_t len)
{
	size_t count = 0;
 ...
 */
static void fix_up_readers(struct mtk_logger_log *log, size_t len)
{
	size_t old = log->w_off;
	size_t new = logger_offset(old + len);
	struct mtk_logger_reader *reader;
 ...
 * The caller needs to hold log->lock.
 */
static void do_write_log(struct mtk_logger_log *log, const void *buf, size_t count)
{
	size_t len;
 ...
 */
static ssize_t do_write_log_from_user(struct mtk_logger_log *log, const void __user *buf, size_t count)
{
	size_t len;
...
{
	struct mtk_logger_log *log = file_get_log(iocb->ki_filp);
...
static struct mtk_logger_log *get_log_from_minor(int);
...
static int logger_open(struct inode *inode, struct file *file)
{
	...
	if (file->f_mode & FMODE_READ) {
		...
		reader = kmalloc(sizeof(struct mtk_logger_reader), GFP_KERNEL);
		if (!reader)
			return -ENOMEM;
...
{
	struct mtk_logger_log *log = file_get_log(file);
	struct mtk_logger_reader *reader;
	long ret = -ENOTTY;
	unsigned long flags;
...
static struct mtk_logger_log VAR = {			\
	.buffer = _buf_ ## VAR,				\
	.misc = {					\
...
/* } mingjian, 20101208 */
static struct mtk_logger_log *get_log_from_minor(int minor)
{
...
static int __init init_log(struct mtk_logger_log *log)
{
	int ret;
...
static ssize_t xlog_logger_kernel_write(struct mtk_logger_log *log, const char *msg, size_t len)
{
	struct logger_entry header;
	struct timespec now;
	ssize_t ret = 0;
...
int __xlog_ksystem_printk(const struct xlog_record *rec, ...)
{
	...
	unsigned long flags;
	int tag_len, n;
	struct mtk_logger_log *log = &log_ksystem;
