...
void disp_pat_clear_wait_reg_flag(void)
{
	g_disp_mutex_reg_update = 0;
}
...
int disp_path_config_OVL_WDMA_path(int mutex_id, int isolated, int connect)
{
	if (isolated) {
	/* mutex module */
#if DDP_ALWAYS_DECOUPLE_PQ_EN
	DISP_REG_SET(DISP_REG_CONFIG_MUTEX_MOD(mutex_id), 0x24c);	/* ovl, wdma1 */
#else
	...
	/* ovl mout */
#if DDP_ALWAYS_DECOUPLE_PQ_EN
	DISP_REG_SET(DISP_REG_CONFIG_OVL_MOUT_EN, 1 << 2);	/* ovl_mout output to color */
	DISP_REG_SET(DISP_REG_CONFIG_DUMMY, 0x1); /* wdma1 input from bls*/
#else
	DISP_REG_SET(DISP_REG_CONFIG_OVL_MOUT_EN, 1 << 0);	/* ovl_mout output to wdma1 */
#endif
	} else {
...
#if DDP_ALWAYS_DECOUPLE_PQ_EN
		ovl_mem_out_done = 0;
		disp_register_irq(DISP_MODULE_WDMA1, disp_path_wait_ovl_wdma_callbak);
#endif
...
#if DDP_ALWAYS_DECOUPLE_PQ_EN
		disp_unregister_irq(DISP_MODULE_WDMA1, disp_path_wait_ovl_wdma_callbak);
#endif
	}
...
int disp_path_config_mutex(struct disp_path_config_struct *pConfig, int mutexId)
{
	...
	if (pConfig->srcModule == DISP_MODULE_RDMA0) {
		...
		mutexValue = 1 << 7;	/* rdma0=7, BLS=9(for backlight pwm control when hdmi connect) */
