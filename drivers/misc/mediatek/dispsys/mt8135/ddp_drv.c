	...
	case MT8135_DISP_RDMA0_IRQ_ID:
		...
		if (reg_val & (1 << 3)) {
			if (cnt_rdma_abnormal < 100) {
				...
				cnt_rdma_abnormal++;
				if ((cnt_rdma_abnormal % 30) == 0) {
					DISP_ERR("rdma0 con0: 0x%x, con1: 0x%x, pitch: 0x%x, addr: 0x%x\n",
					...
					DISP_ERR("MMsystemConfig con0: 0x%x, con1: 0x%x\n",
...
static long disp_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_IOCTL_WRITE_REG:
		...
		if ((wParams.reg >= DISPSYS_ROT_BASE && wParams.reg < DISPSYS_ROT_BASE + 0x1000) ||
			(wParams.reg >= DISPSYS_SCL_BASE && wParams.reg < DISPSYS_SCL_BASE + 0x1000) ||
			(wParams.reg >= DISPSYS_WDMA0_BASE && wParams.reg < DISPSYS_WDMA0_BASE + 0x1000) ||
			(wParams.reg >= DISPSYS_TDSHP_BASE && wParams.reg < DISPSYS_TDSHP_BASE + 0x1000) ||
			(wParams.reg >= DISPSYS_COLOR_BASE && wParams.reg < DISPSYS_COLOR_BASE + 0x1000) ||
			(wParams.reg >= DISPSYS_BLS_BASE && wParams.reg < DISPSYS_BLS_BASE + 0x1000) ||
			(wParams.reg >= DISPSYS_MUTEX_BASE && wParams.reg < DISPSYS_MUTEX_BASE + 0x1000)  ||
			(wParams.reg >= DISPSYS_CONFIG_BASE && wParams.reg < DISPSYS_CONFIG_BASE + 0x1000)) {
			/**(volatile unsigned int *)wParams.reg =
		    (*(volatile unsigned int *)wParams.reg & ~wParams.mask) | (wParams.
									       val & wParams.mask);*/
			mt65xx_reg_sync_writel((DISP_REG_GET(wParams.reg) & ~wParams.mask) |
				(wParams.val & wParams.mask) , wParams.reg);
		} else{
	...
	case DISP_IOCTL_READ_REG:
		...
		if ((rParams.reg >= DISPSYS_ROT_BASE && rParams.reg < DISPSYS_ROT_BASE + 0x1000) ||
		(rParams.reg >= DISPSYS_SCL_BASE && rParams.reg < DISPSYS_SCL_BASE + 0x1000) ||
		(rParams.reg >= DISPSYS_WDMA0_BASE && rParams.reg < DISPSYS_WDMA0_BASE + 0x1000) ||
		(rParams.reg >= DISPSYS_TDSHP_BASE && rParams.reg < DISPSYS_TDSHP_BASE + 0x1000) ||
		(rParams.reg >= DISPSYS_COLOR_BASE && rParams.reg < DISPSYS_COLOR_BASE + 0x1000) ||
		(rParams.reg >= DISPSYS_BLS_BASE && rParams.reg < DISPSYS_BLS_BASE + 0x1000) ||
		(rParams.reg >= DISPSYS_MUTEX_BASE && rParams.reg < DISPSYS_MUTEX_BASE + 0x1000) ||
		(rParams.reg >= DISPSYS_CONFIG_BASE && rParams.reg < DISPSYS_CONFIG_BASE + 0x1000)) {
			value = DISP_REG_GET(rParams.reg) & rParams.mask;

			DISP_DBG("read 0x%x = 0x%x (0x%x)\n", rParams.reg, value, rParams.mask);
		} else {
			DISP_ERR("reg read, addr invalid, add=0x%x\n", rParams.reg);
			return -EFAULT;
		}
	...
	case DISP_IOCTL_GET_OVL:
		...
		if (layer >= DDP_OVL_LAYER_MUN || layer < 0) {
			DISP_ERR("DISP_IOCTL_GET_OVL, invalid overlay layer\n");
			return -EFAULT;
		}
