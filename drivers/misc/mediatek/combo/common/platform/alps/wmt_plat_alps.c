...
static INT32 wmt_plat_dump_pin_conf(VOID)
{
	...
	if (pdata->eint.all.valid)
		WMT_DBG_FUNC("ALL_EINT(GPIO%d)\n", pdata->eint.all.pin);
	else
		WMT_DBG_FUNC("ALL_EINT(not defined)\n");
		...
		if (pdata->eint.all.valid) {
			...
			if (PIN_STA_INIT == state) {
				mt_eint_registration(eint,
						     pdata->eint.all.flags, wmt_plat_all_eirq_cb,
						     0);
				mt_eint_mask(eint);
...
INT32 wmt_plat_pmu_ctrl(ENUM_PIN_STATE state)
{
	...
	case PIN_STA_DEINIT:
		...
		if (-1 != pmuv28->pin) {
			...
			mt_pin_set_pull(pmuv28->pin, MT_PIN_PULL_ENABLE_DOWN);
		}
...
INT32 wmt_plat_pcm_ctrl(ENUM_PIN_STATE state)
{
	...
	if (conf->merge) {
		...
		default:
			WMT_WARN_FUNC("WMT-PLAT:<Merge IF>Warnning, invalid state(%d) "
				      "on PCM Group\n", state);
			break;
		}
	} else
...
skip_merge_if:
	...
	default:
		WMT_WARN_FUNC("WMT-PLAT:MT6589 Warnning, invalid state(%d) "
			      "on PCM Group\n", state);
		break;
	}
...
INT32 wmt_plat_i2s_ctrl(ENUM_PIN_STATE state)
{
	...
	if (!i2s_ck->valid || !i2s_ws->valid || !i2s_dat->valid || !pcm_out->valid)
		return 0;
	...
	if (conf->merge) {
		...
		if (conf->fm_digital) {
			...
			case PIN_STA_DEINIT:
				...
				break;
			default:
				WMT_WARN_FUNC("WMT-PLAT:<Merge IF>Warnning, invalid "
...
skip_merge_if:
	...
	if (conf->fm_digital) {
		...
		case PIN_STA_DEINIT:
			...
			break;
		default:
			WMT_WARN_FUNC("WMT-PLAT:<I2S IF>Warnning, invalid state(%d) "
				      "on I2S Group\n", state);
			break;
		}
	} else
...
static INT32 wmt_plat_gps_lna_ctrl(ENUM_PIN_STATE state)
{
	...
	if (!lna->valid) {
		WMT_DBG_FUNC("host gps lna pin not defined\n");
		return 0;
	}
...
static INT32 wmt_plat_uart_rx_ctrl(ENUM_PIN_STATE state)
{
	...
	if (!rxd->valid) {
		WMT_WARN_FUNC("host uart rx pin not defined\n");
		return 0;
	}
	...
	case PIN_STA_IN_L:
	case PIN_STA_DEINIT:
		mt_pin_set_pull(rxd->pin, MT_PIN_PULL_DISABLE);
		gpio_direction_output(rxd->pin, 0);
		mt_pin_set_mode_gpio(rxd->pin);
		break;
	case PIN_STA_IN_NP:
		mt_pin_set_mode_gpio(rxd->pin);
		gpio_direction_input(rxd->pin);
		mt_pin_set_pull(rxd->pin, MT_PIN_PULL_DISABLE);
		break;
	case PIN_STA_OUT_H:
		mt_pin_set_pull(rxd->pin, MT_PIN_PULL_ENABLE_UP);
		mt_pin_set_mode_gpio(rxd->pin);
		gpio_direction_output(rxd->pin, 1);
		break;
	case PIN_STA_OUT_L:
		mt_pin_set_mode_gpio(rxd->pin);
		gpio_direction_output(rxd->pin, 0);
...
INT32 wmt_plat_merge_if_flag_ctrl(UINT32 enable)
{
	...
	if (enable) {
		if (pdata->conf.merge)
			...
			WMT_WARN_FUNC
			    ("neither MT6589, MTK_MERGE_INTERFACE_SUPPORT nor MT6628 is not set to true, ");
...
int wmt_plat_alps_init(struct mtk_combo_data *wmt_data)
{
	...
	if (pins->pcm.sync.valid &&
	    pins->pcm.clk.valid && pins->pcm.in.valid && pins->pcm.out.valid) {
