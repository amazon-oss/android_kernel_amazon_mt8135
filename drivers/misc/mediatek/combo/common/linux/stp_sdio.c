		...
		if (OWN_SET == stp_sdio_get_own_state()) {
			...
			if (0 == stp_sdio_do_own_clr(0)) {
				...
				if ((0 != pInfo->sleep_flag) || (0 != pInfo->wakeup_flag)) {
					pInfo->wakeup_flag = 0;
					pInfo->sleep_flag = 0;
					STPSDIO_ERR_FUNC("clr fw own fail, signal for sleep/wakeup to return\n");
		...
		if (1000 < while_loop_counter) {
			...
			if ((0 != pInfo->sleep_flag) || (0 != pInfo->wakeup_flag)) {
				...
				if (STP_SDIO_TX_PKT_MAX_CNT == pInfo->firmware_info.tx_packet_num) {
					STPSDIO_ERR_FUNC
					    ("STP tx window full, proberbly STP-SDIO cannot handle tx request. tx_packet_num:%d\n",
...
#if KMALLOC_UPDATE
			*(gp_info->pkt_buf.tx_buf +
			  gp_info->pkt_buf.wr_idx * STP_SDIO_TX_ENTRY_SIZE + 0) = (UINT8) ((size + STP_SDIO_HDR_SIZE) & 0xff);
			*(gp_info->pkt_buf.tx_buf +
			gp_info->pkt_buf.wr_idx * STP_SDIO_TX_ENTRY_SIZE + 1) = (UINT8) ((size + STP_SDIO_HDR_SIZE) >> 8);
			gp_info->pkt_buf.tx_buf_ts[gp_info->pkt_buf.wr_idx] = jiffies;
...
static VOID stp_sdio_txdbg_dump(VOID)
{
	...
	for (i = 0; i < STP_SDIO_TXDBG_COUNT; ++i) {
		...
		if (0 == len) {
			STPSDIO_DBG_FUNC("idx(%x) 0 == len dump skip\n", idx);
			continue;
		}

		len = len > STP_SDIO_TXDBG_MAX_SIZE ? STP_SDIO_TXDBG_MAX_SIZE : len;
		STPSDIO_DBG_FUNC("stp_sdio_txdbg_buffer idx(%x) bus_txlen(0x%x, %d) ts(%d)\n",
		...
		for (j = 0; j < STP_SDIO_TX_ENTRY_SIZE && j < len; j += 16) {
			...
			STPSDIO_DBG_FUNC("[0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x \
		...
		STPSDIO_DBG_FUNC("stp_sdio_txdbg_buffer dump ok\n");
	}
	...
	STPSDIO_DBG_FUNC("Dump tx info: pkt_num(%d) fifo(%d) pkt_list.rd(0x%x, %ld) pkt_list.wr(0x%x, %ld)\n",
	...
	for (i = 0; i < STP_SDIO_TX_PKT_LIST_SIZE; ++i) {
		...
		STPSDIO_DBG_FUNC("tx_pkt_list idx(0x%x, %d) size(0x%x, %d), in_ts(%d), out_ts(%d)\n",
	...
	for (i = 0; i < STP_SDIO_TX_BUF_CNT; ++i) {
...
#endif
		STPSDIO_DBG_FUNC("pkt_buf.tx_buf idx(%x) ts(%d) len(%d)\n", idx,
		...
		STPSDIO_DBG_FUNC("pkt_buf.tx_buf dump ok\n");
	}
