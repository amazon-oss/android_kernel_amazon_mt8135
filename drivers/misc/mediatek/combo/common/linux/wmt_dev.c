...
static void wmt_dev_early_suspend(struct early_suspend *h)
{
	...
	if (0x6630 != mtk_wcn_wmt_chipid_query()) {
		...
		WMT_INFO_FUNC("chip is always power on\n");
	}
}
...
#define COUNT_THRESHOLD_TO_TEMP_QUERY 200
	...
	WMT_DBG_FUNC("**poll_during_time = %u > %u, sdio_during_count = %u > %u, query\n",
		      jiffies_to_msecs(poll_during_time), TIME_THRESHOLD_TO_TEMP_QUERY,
...
static void wmt_shutdown(struct platform_device *pdev)
{
	WMT_WARN_FUNC("enter %s\n", __func__);
	mtk_wcn_wmt_func_off(WMTDRV_TYPE_LPBK);
	mtk_wcn_wmt_func_off(WMTDRV_TYPE_BT);
	mtk_wcn_wmt_func_off(WMTDRV_TYPE_WIFI);
	WMT_WARN_FUNC("exit %s\n", __func__);
}
...
static int wmt_driver_init(void)
{
	return platform_driver_register(&wmt_driver);
}

static void wmt_driver_exit(void)
{
	platform_driver_unregister(&wmt_driver);
}
...
{
	return wmt_driver_init();

}

VOID mtk_wcn_combo_common_drv_exit(VOID)
{
	return wmt_driver_exit();
}
...
#else
module_init(wmt_driver_init);
module_exit(wmt_driver_exit);
#endif
