...
0xD4, 0x03, 0x09, 0x02, 0x00, 0x00, 0x00,
...
0xfc, 0x08, 0x09, 0x02, 0x00, 0x00, 0x08,
0x00, 0xff, 0xff, 0xff, 0xff };
...
static INT32 mt6630_patch_dwn(UINT32 index)
{
	...
	if (index == 0) {
		WMT_DBG_FUNC("===========================================\n");
		WMT_DBG_FUNC("[Combo Patch] Built Time = %s\n", cDataTime);
		WMT_DBG_FUNC("[Combo Patch] Hw Ver = 0x%x\n",
			      ((u2HwVer & 0x00ff) << 8) | ((u2HwVer & 0xff00) >> 8));
		WMT_DBG_FUNC("[Combo Patch] Sw Ver = 0x%x\n",
			      ((u2SwVer & 0x00ff) << 8) | ((u2SwVer & 0xff00) >> 8));
		WMT_DBG_FUNC("[Combo Patch] Ph Ver = 0x%04x\n",
			      ((u4PatchVer & 0xff000000) >> 24) | ((u4PatchVer & 0x00ff0000) >>
								   16));
		WMT_DBG_FUNC("[Combo Patch] Platform = %c%c%c%c\n", patchHdr->ucPLat[0],
