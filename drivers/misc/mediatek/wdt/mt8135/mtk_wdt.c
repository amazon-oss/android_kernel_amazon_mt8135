...
void mtk_wdt_set_irq_affinity(int cpu)
{
	char str_buf[128];

	snprintf(str_buf, sizeof(str_buf), "CPU%d hard LOCKUP. Routed WD FIQ to it!!!\n", cpu);
	aee_sram_fiq_log(str_buf);

	if (kree_set_fiq_affinity(AP_RGU_WDT_IRQ_ID, cpu))
		pr_err("[WDT] route WD IRQ to %d failed!!!\n", cpu);
}
