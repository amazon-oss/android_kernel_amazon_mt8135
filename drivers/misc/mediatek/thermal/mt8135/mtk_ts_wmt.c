...
static int mtktswmt_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct mtktswmt_thermal_zone *tzone;
	struct mtk_thermal_platform_data_wrapper *wdata = dev_get_platdata(&pdev->dev);
	struct mtk_thermal_platform_data *pdata = wdata->data;
	struct thermal_dev_params *mtktswmt_tdp = &(wdata->params);
