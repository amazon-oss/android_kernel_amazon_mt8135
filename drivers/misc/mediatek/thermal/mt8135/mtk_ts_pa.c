...
static int __init mtktspa_init(void)
{
	...
	if (!mtktspa_dir) {
		mtktspa_dprintk("[mtktspa_init]: mkdir /proc/driver/thermal failed\n");
	} else {
		entry = proc_create("tzpa", S_IRUGO | S_IWUSR, mtktspa_dir, &mtktspa_fops);
		if (!entry)
			mtktspa_dprintk("[mtktspa_init]: create tzpa failed\n");
	}
