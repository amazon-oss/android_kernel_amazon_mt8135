...
static int thermal_budget_probe(struct platform_device *pdev)
{
	...
	device_create(tdev->cls, NULL, MKDEV(tdev->major, 0), NULL, "thermal_budget");
	...
	tdev->pdata = pdata;
	_thermal_budget = _thermal_budget_cpu = _thermal_budget_cooler = MAXIMUM_CPU_POWER;
	return result;
}
...
static struct mtk_cooler_platform_data thermal_budget_pdata = {
	.type = "thermal_budget",
	.state = 0,
...
static struct platform_device thermal_budget_device = {
	...
	.id = -1,
	.dev = {
		.platform_data = &thermal_budget_pdata,
	},
};
