...
static void set_jeita_charging_current(void)
{
	...
	if (g_temp_status == TEMP_NEG_10_TO_POS_0) {
		...
		g_temp_CC_value = JEITA_TEMP_0_10_FAST_CHR_CUR;
		if (trickle_charge_stage == KAL_TRUE)
			g_temp_CC_value = JEITA_TEMP_0_10_FAST_CHR_CUR2;
	} else if (g_temp_status == TEMP_POS_10_TO_POS_45) {
		if (BMT_status.temperature <= 23)
			g_temp_CC_value = JEITA_TEMP_10_23_FAST_CHR_CUR;
		else
			g_temp_CC_value = JEITA_TEMP_23_45_FAST_CHR_CUR;
	} else {
		g_temp_CC_value = JEITA_TEMP_45_60_FAST_CHR_CUR;
	}
}
