...
static ssize_t show_Custom_RTC_SOC(struct device *dev, struct device_attribute *attr,
					    char *buf)
{
	battery_xlog_printk(BAT_LOG_CRTI, "custom rtc soc = %d\n",
			    g_custom_rtc_soc);
	return sprintf(buf, "%d\n", g_custom_rtc_soc);
}
...
static ssize_t levels_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	struct power_supply *psy = cdev->devdata;
	struct battery_data *data = container_of(psy, struct battery_data, psy);
	struct mtk_cooler_platform_data *cool_dev = data->cool_dev;
	int i = 0;
	int offset = 0;

	if (!psy || !data || !cool_dev)
		return -EINVAL;
...
static int filter_battery_temperature(int instant_temp)
{
	...
	for (check_count = 0; check_count < 3; check_count++) {
		if (instant_temp > MAX_DISCHARGE_TEMP || instant_temp < MIN_DISCHARGE_TEMP) {
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
		if (BMT_status.temperature > MAX_DISCHARGE_TEMP || BMT_status.temperature < MIN_DISCHARGE_TEMP) {
...
void BAT_thread(void)
{
	if (battery_meter_initilized == KAL_FALSE) {
		...
		gFG_CV_Battery_Voltage = JEITA_TEMP_POS_10_TO_POS_45_CV_VOLTAGE;
...
static int battery_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	struct battery_data *data = thermal->devdata;
	if (!data)
		return -EINVAL;
	*t = (unsigned long)data->BAT_TEMP * 100;
	return 0;
}

static int battery_get_mode(struct thermal_zone_device *thermal, enum thermal_device_mode *mode)
{
	struct battery_thermal_zone *tdata;
	struct battery_data *data = thermal->devdata;
...
static int battery_set_mode(struct thermal_zone_device *thermal, enum thermal_device_mode mode)
{
	struct battery_thermal_zone *tdata;
	struct battery_data *data = thermal->devdata;
	...
	if (!tdata)
		return -EINVAL;
	tdata->mode = mode;
	schedule_work(&tdata->therm_work);
	return 0;
}

static int battery_get_trip_type(struct thermal_zone_device *thermal,
				 int trip,
				 enum thermal_trip_type *type)
{
	...
	if (!tdata)
		return -EINVAL;
	*type = tdata->trips[trip].type;
	return 0;
}

static int battery_get_trip_temp(struct thermal_zone_device *thermal,
				 int trip,
				 unsigned long *t)
{
	struct battery_thermal_zone *tdata;
	...
	if (!tdata)
		return -EINVAL;
	*t = tdata->trips[trip].temp;
	return 0;
}

static int battery_set_trip_temp(struct thermal_zone_device *thermal,
				  int trip,
				  unsigned long t)
{
	struct battery_thermal_zone *tdata;
	...
	if (!tdata)
		return -EINVAL;
	tdata->trips[trip].temp =t;
	return 0;
}

static int battery_get_crit_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	*t = BATTERY_TEMP_CRIT;
	return 0;
}

static int battery_thermal_notify(struct thermal_zone_device *thermal,
				int trip,
				enum thermal_trip_type type)
{
	return 0;
...
static struct thermal_zone_device_ops battery_dev_ops = {
	.get_temp = battery_get_temp,
	.get_mode = battery_get_mode,
	.set_mode = battery_set_mode,
	.get_trip_type = battery_get_trip_type,
	.get_trip_temp = battery_get_trip_temp,
	.set_trip_temp = battery_set_trip_temp,
	.get_crit_temp = battery_get_crit_temp,
	.notify = battery_thermal_notify,
};
...
static void battery_work(struct work_struct *work)
{
	...
	struct power_supply *psy = &(battery_main.psy);

	tdata = container_of(work, struct battery_thermal_zone, therm_work);
	if (!tdata)
		return;
	mode = tdata->mode;
	if (mode == THERMAL_DEVICE_DISABLED)
		return;
	if (psy->tzd)
		thermal_zone_device_update(psy->tzd);
}

static int battery_read_temp(struct thermal_dev *tdev)
{
	struct power_supply *psy = &(battery_main.psy);
	struct battery_data *data = container_of(psy, struct battery_data, psy);
	...
	if (!data)
		return -EINVAL;
	return data->BAT_TEMP * 100;
}

static struct thermal_dev_ops battery_fops = {
	.get_temp = battery_read_temp,
};

static int battery_show_params(struct device *dev,
			       struct device_attribute *devattr,
			       char *buf)
{
	struct thermal_zone_device *thermal = container_of(dev, struct thermal_zone_device, device);
	struct battery_data *data = thermal->devdata;
	struct battery_thermal_zone *tdata = data->tdata;

	if (!tdata)
		return -EINVAL;
	return sprintf(buf, "offset=%d alpha=%d weight=%d\n",
		       tdata->therm_fw->tdp->offset,
		       tdata->therm_fw->tdp->alpha,
		       tdata->therm_fw->tdp->weight);
}

static ssize_t battery_store_params(struct device *dev,
				    struct device_attribute *devattr,
				    const char *buf,
				    size_t count)
{
	...
	if (sscanf(buf, "%19s %d", param, &value) == 2) {
		if (!strcmp(param, "offset"))
			tdata->therm_fw->tdp->offset = value;
		if (!strcmp(param, "alpha"))
			tdata->therm_fw->tdp->alpha = value;
		if (!strcmp(param, "weight"))
			tdata->therm_fw->tdp->weight = value;
		return count;
	}
	return -EINVAL;
}
static DEVICE_ATTR(params, S_IRUGO | S_IWUSR, battery_show_params, battery_store_params);

static int battery_register_thermal(struct device *dev)
{
	int ret;
	struct mt_battery_charging_custom_data_wrapper *wdata = dev_get_platdata(dev);
	mt_battery_charging_custom_data *pdata = wdata->data;
	struct battery_thermal_zone *tdata = &(pdata->tdata);
	struct power_supply *psy = &(battery_main.psy);
	struct thermal_dev_params *battery_tdp = &(wdata->params);
	...
	psy->tzd = thermal_zone_device_register(psy->name,
						tdata->num_trips,
						1,
						&battery_main,
						&battery_dev_ops,
						...
						tdata->polling_delay);
	if (IS_ERR(psy->tzd)) {
		pr_err("%s Failed to register battery thermal zone device\n", __func__);
		return -EINVAL;
	}

	tdata->therm_fw = kzalloc(sizeof(struct thermal_dev), GFP_KERNEL);
	if (!tdata->therm_fw)
		return -ENOMEM;

	tdata->therm_fw->name = "battery";
	tdata->therm_fw->dev = dev;
	tdata->therm_fw->dev_ops = &battery_fops;
	tdata->therm_fw->tdp = battery_tdp;

	ret = thermal_dev_register(tdata->therm_fw);
	if (ret) {
		pr_err("Error registering therml battery device\n");
		return -EINVAL;
	}

	tdata->mode = THERMAL_DEVICE_ENABLED;
	INIT_WORK(&tdata->therm_work, battery_work);
	ret = device_create_file(&psy->tzd->device, &dev_attr_params);
	if (ret)
		pr_err("%s Failed to create params attr\n", __func__);
	device_create_file(&psy->tcd->device, &dev_attr_levels);
	return 0;
}
...
static int battery_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		battery_main.cool_dev = &(pdata->cool_dev);
		battery_main.tdata = &(pdata->tdata);
	}
	...
	ret = battery_register_thermal(dev);
	if (ret) {
		pr_err("%s Failed to battery_register_thermal\n", __func__);
		return ret;
	}
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(dev, &dev_attr_Custom_RTC_SOC);
	}
	...
	BMT_status.cv_voltage_changed = KAL_FALSE;
