...
#define CV_BATTERY_VOLTAGE_MIN     3900000
#define CV_BATTERY_VOLTAGE_MAX     4200000
...
kal_int32 gFG_CV_Battery_Voltage = 0;
kal_int32 gFG_CV_Voltage_Reduction_Supported = 0;
#endif				/* battery info */
...
kal_int32 fgauge_get_Q_max(kal_int16 temperature)
{
	...
	if (temperature <= TEMPERATURE_T1) {
		...
		if (battery_profile_t1_5) {
			low_temperature = TEMPERATURE_T1_5;
			low_Q_max = Q_MAX_POS_10;
		} else {
			low_temperature = TEMPERATURE_T1;
			low_Q_max = Q_MAX_POS_0;
		}
...
kal_int32 fgauge_get_Q_max_high_current(kal_int16 temperature)
{
	...
	if (temperature <= TEMPERATURE_T1) {
		...
		if (battery_profile_t1_5) {
			low_temperature = TEMPERATURE_T1_5;
			low_Q_max = Q_MAX_POS_10_H_CURRENT;
		} else {
			low_temperature = TEMPERATURE_T1;
			low_Q_max = Q_MAX_POS_0_H_CURRENT;
		}
...
void fgauge_construct_battery_profile(kal_int32 temperature, BATTERY_PROFILE_STRUC_P temp_profile_p)
{
	...
	if (temperature <= TEMPERATURE_T1) {
		...
		if (battery_profile_t1_5)
			low_profile_p = fgauge_get_profile(TEMPERATURE_T1_5);
		else
			low_profile_p = fgauge_get_profile(TEMPERATURE_T1);
...
void fgauge_construct_r_table_profile(kal_int32 temperature, R_PROFILE_STRUC_P temp_profile_p)
{
	...
	if (temperature <= TEMPERATURE_T1) {
		...
		if (r_profile_t1_5)
			low_profile_p = fgauge_get_profile_r_table(TEMPERATURE_T1_5);
		else
			low_profile_p = fgauge_get_profile_r_table(TEMPERATURE_T1);
...
void fgauge_construct_table_by_temp(void)
{
	...
	if (avg_temp != last_temp) {
		bm_print(BM_LOG_FULL,
			 ...
			 last_temp, avg_temp);
		if (NON_LINEAR_BAT_R) { /* handle non-linear R for Aston SDI cell */
...
kal_int32 fgauge_get_Q_max_high_current_by_current(kal_int32 i_current, kal_int16 val_temp)
{
	...
	if (NON_LINEAR_BAT_R) /* for Aston SDI cell */
		threshold = 3380;
	else
		threshold = p_bat_meter_data->power_off_threshold;
...
kal_int32 battery_meter_trans_battery_percentage(kal_int32 d_val)
{
	...
	if (KAL_FALSE == gFG_Is_Charging && g_currentfactor > 100) {
		...
		/* only compensate up to 0.8A for Aston SDI cell */
		if (NON_LINEAR_BAT_R && i_avg_current > 8000)
				     ...
				     const char *buf, size_t size)
{
	...
	if (1 == sscanf(buf, "%d", &cv_voltage)) {
		if ((cv_voltage <= CV_BATTERY_VOLTAGE_MAX) && (cv_voltage >= CV_BATTERY_VOLTAGE_MIN)
			...
			BMT_status.cv_voltage_changed = KAL_TRUE;
		} else {
...
#define BATTERY_CVR_ATL_PROFILE_COUNT      8
#define BATTERY_CVR_SDI_PROFILE_COUNT      10
#define BATTERY_CVR_ATL_Q_MAX_COUNT        8
#define BATTERY_CVR_SDI_Q_MAX_COUNT        10
#define BATTERY_CVR_CV_COUNT               6

static ssize_t copy_atl_profile(BATTERY_PROFILE_STRUC * aston_atl_custom_battery_profile_array[],
		R_PROFILE_STRUC * aston_atl_custom_r_profile_array[], const char *buffer, int count, int profile_count)
{
	if (profile_count < (BATTERY_CVR_ATL_PROFILE_COUNT / 2)) {
		if (copy_from_user(aston_atl_custom_battery_profile_array[profile_count],
			buffer, count)) {
			pr_err("%s, aston_atl_custom_battery_profile_array copy from user fail\n", __func__);
			return -EFAULT;
		}
	} else {
		if (copy_from_user(aston_atl_custom_r_profile_array[profile_count
						- (BATTERY_CVR_ATL_PROFILE_COUNT / 2)],
			buffer, count)) {
			pr_err("%s, aston_atl_custom_r_profile_array copy from user fail\n", __func__);
			return -EFAULT;
		}
	}
...
static ssize_t copy_sdi_profile(BATTERY_PROFILE_STRUC * aston_sdi_custom_battery_profile_array[],
		R_PROFILE_STRUC * aston_sdi_custom_r_profile_array[], const char *buffer, int count, int profile_count)
{
	if (profile_count < (BATTERY_CVR_SDI_PROFILE_COUNT / 2)) {
		if (copy_from_user(aston_sdi_custom_battery_profile_array[profile_count],
			buffer, count)) {
			pr_err("%s, aston_sdi_custom_battery_profile_array copy from user fail\n", __func__);
			return -EFAULT;
		}
	} else {
		if (copy_from_user(aston_sdi_custom_r_profile_array[profile_count
						- (BATTERY_CVR_SDI_PROFILE_COUNT / 2)],
			buffer, count)) {
			pr_err("%s, aston_sdi_custom_r_profile_array copy from user fail\n", __func__);
			return -EFAULT;
		}
	}
...
static void battery_meter_data_init(int count)
{
	...
	for (i = 0; i < (count / sizeof(BATTERY_PROFILE_STRUC)); i++) {
		aston_atl_custom_battery_profile_temperature[i].percentage = 0;
		aston_atl_custom_battery_profile_temperature[i].voltage = 0;
		aston_atl_custom_r_profile_temperature[i].resistance = 0;
		aston_atl_custom_r_profile_temperature[i].voltage = 0;

		aston_sdi_custom_battery_profile_temperature[i].percentage = 0;
		aston_sdi_custom_battery_profile_temperature[i].voltage = 0;
		aston_sdi_custom_r_profile_temperature[i].resistance = 0;
		aston_sdi_custom_r_profile_temperature[i].voltage = 0;
	}
	pr_info("%s copy from user pass\n", __func__);
}

static ssize_t battery_cvr_atl_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	static int profile_count;

	pr_info("%s start with profile count %d\n", __func__, profile_count);
	if ((gFG_CV_Voltage_Reduction_Supported == 0) || (count < (ATL_BATTERY_3900_PROFILE_NUM * 8))
		|| (profile_count >= BATTERY_CVR_ATL_PROFILE_COUNT)) {
		pr_err("%s, Not valid data return\n", __func__);
		return -EFAULT;
	}

	if (copy_atl_profile(aston_atl_custom_battery_profile_array,
		aston_atl_custom_r_profile_array, buffer, count, profile_count) != count) {
	...
	if (profile_count == BATTERY_CVR_ATL_PROFILE_COUNT) {
		battery_meter_data_init(count);
		profile_count = 0;
	}

	pr_info("%s end with profile count %d\n", __func__, profile_count);
	return count;
}

static ssize_t battery_cvr_sdi_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	static int profile_count;

	pr_info("%s start with profile count %d\n", __func__, profile_count);
	if ((gFG_CV_Voltage_Reduction_Supported == 0) || (profile_count >= BATTERY_CVR_SDI_PROFILE_COUNT)
		|| (count < (SDI_BATTERY_3900_PROFILE_NUM * 8))) {
		pr_err("%s, Not valid data return\n", __func__);
		return -EFAULT;
	}

	if (copy_sdi_profile(aston_sdi_custom_battery_profile_array,
		aston_sdi_custom_r_profile_array, buffer, count, profile_count) != count) {
		pr_err("%s, Copy from user fail\n", __func__);
		return -EFAULT;
	}
	...
	if (profile_count == BATTERY_CVR_SDI_PROFILE_COUNT) {
		battery_meter_data_init(count);
		profile_count = 0;
	}
...
static ssize_t battery_cvr_atl_q_max_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int q_max[BATTERY_CVR_ATL_Q_MAX_COUNT];

	if ((gFG_CV_Voltage_Reduction_Supported == 0) || (count != (BATTERY_CVR_ATL_Q_MAX_COUNT * 4))) {
		pr_err("%s, Not valid data return\n", __func__);
		return -EFAULT;
	}
	...
	mt_bat_meter_data.q_max_pos_50 = q_max[0];
	mt_bat_meter_data.q_max_pos_50_h_current = q_max[1];
	mt_bat_meter_data.q_max_pos_25 = q_max[2];
	mt_bat_meter_data.q_max_pos_25_h_current = q_max[3];
	mt_bat_meter_data.q_max_pos_0 = q_max[4];
	mt_bat_meter_data.q_max_pos_0_h_current = q_max[5];
	mt_bat_meter_data.q_max_neg_10 = q_max[6];
	mt_bat_meter_data.q_max_neg_10_h_current = q_max[7];

	pr_info("%s copy from user pass\n", __func__);
	return count;
}

static ssize_t battery_cvr_sdi_q_max_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int q_max[BATTERY_CVR_SDI_Q_MAX_COUNT];

	if ((gFG_CV_Voltage_Reduction_Supported == 0) || (count != (BATTERY_CVR_SDI_Q_MAX_COUNT * 4))) {
		pr_err("%s, Not valid data return\n", __func__);
		return -EFAULT;
	}
	...
	mt_bat_meter_data.q_max_pos_25 = q_max[2];
	mt_bat_meter_data.q_max_pos_25_h_current = q_max[3];
	mt_bat_meter_data.q_max_pos_10 = q_max[4];
	mt_bat_meter_data.q_max_pos_10_h_current = q_max[5];
	mt_bat_meter_data.q_max_pos_0 = q_max[6];
	mt_bat_meter_data.q_max_pos_0_h_current = q_max[7];
	mt_bat_meter_data.q_max_neg_10 = q_max[8];
	mt_bat_meter_data.q_max_neg_10_h_current = q_max[9];

	pr_info("%s copy from user pass\n", __func__);
	return count;
}

static ssize_t battery_cvr_cv_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int cv[BATTERY_CVR_CV_COUNT];

	if ((gFG_CV_Voltage_Reduction_Supported == 0) || (count != (BATTERY_CVR_CV_COUNT * 4))) {
		pr_err("%s, Not valid data return\n", __func__);
		return -EFAULT;
	}

	if (copy_from_user(cv, buffer, count)) {
		pr_err("%s, Copy from user fail\n", __func__);
		return -EFAULT;
	}

	mt_bat_charging_data.jeita_temp_above_pos_60_cv_voltage = cv[0];
	mt_bat_charging_data.jeita_temp_pos_45_to_pos_60_cv_voltage = cv[1];
	mt_bat_charging_data.jeita_temp_pos_10_to_pos_45_cv_voltage = cv[2];
	mt_bat_charging_data.jeita_temp_pos_0_to_pos_10_cv_voltage = cv[3];
	mt_bat_charging_data.jeita_temp_neg_10_to_pos_0_cv_voltage = cv[4];
	mt_bat_charging_data.jeita_temp_below_neg_10_cv_voltage = cv[5];

	pr_info("%s copy from user pass\n", __func__);
	return count;
}

static const struct file_operations battery_cvr_atl_proc_fops = {
	.write = battery_cvr_atl_write,
};

static const struct file_operations battery_cvr_sdi_proc_fops = {
	.write = battery_cvr_sdi_write,
};

static const struct file_operations battery_cvr_atl_q_max_proc_fops = {
	.write = battery_cvr_atl_q_max_write,
};

static const struct file_operations battery_cvr_sdi_q_max_proc_fops = {
	.write = battery_cvr_sdi_q_max_write,
};

static const struct file_operations battery_cvr_cv_proc_fops = {
	.write = battery_cvr_cv_write,
};
...
static int init_proc_battery_cvr(void)
{
	...
	if (!battery_cvr_dir) {
		...
		proc_create("battery_cvr_atl", S_IRUGO | S_IWUSR, battery_cvr_dir, &battery_cvr_atl_proc_fops);
		battery_xlog_printk(BAT_LOG_CRTI, "proc_create battery_atl_proc_fops\n");
		proc_create("battery_cvr_sdi", S_IRUGO | S_IWUSR, battery_cvr_dir, &battery_cvr_sdi_proc_fops);
		battery_xlog_printk(BAT_LOG_CRTI, "proc_create battery_sdi_proc_fops\n");

		proc_create("battery_cvr_atl_q_max", S_IRUGO | S_IWUSR, battery_cvr_dir,
						&battery_cvr_atl_q_max_proc_fops);
		battery_xlog_printk(BAT_LOG_CRTI, "proc_create battery_atl_q_max_proc_fops\n");
		proc_create("battery_cvr_sdi_q_max", S_IRUGO | S_IWUSR, battery_cvr_dir,
						&battery_cvr_sdi_q_max_proc_fops);
		battery_xlog_printk(BAT_LOG_CRTI, "proc_create battery_sdi_q_max_proc_fops\n");
		proc_create("battery_cvr_cv", S_IRUGO | S_IWUSR, battery_cvr_dir, &battery_cvr_cv_proc_fops);
		battery_xlog_printk(BAT_LOG_CRTI, "proc_create battery_cv_proc_fops\n");
	}
...
static ssize_t show_battery_vendor(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	bm_print(BM_LOG_CRTI, "battery_module = %u\n", battery_module);

	/* use Ariel/ATL if no valid IDME info */
	if (battery_module == 0)
		battery_module = ATL;
#else
	battery_module = ATL;  /* default module - ATL */
#endif
