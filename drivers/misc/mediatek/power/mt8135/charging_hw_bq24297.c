...
static kal_uint32 charging_hw_init(void *data)
{
	...
	if (p_bat_charging_data->use_4_6_vindpm)
		bq24297_set_vindpm(0x9);	/* VIN DPM check 4.60V */
	else
		bq24297_set_vindpm(0x6);	/* VIN DPM check 4.36V (reset value) */

	bq24297_set_reg_rst(0x0);
	bq24297_set_wdt_rst(0x1);	/* Kick watchdog */
	bq24297_set_sys_min(0x5);	/* Minimum system voltage 3.5V */
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
	prechg_cur = p_bat_charging_data->prechg_cur;
	prechg_cur /= 100;
	BUG_ON(prechg_cur < 128 || prechg_cur > 512);
	bq24297_set_iprechg((prechg_cur - 128) >> 7);	/* Precharge current */
#else
...
#define MAX_CHG_CURRENT 3008
#define CHG_CURRENT_OFF 512
static kal_uint32 charging_set_current(void *data)
{
	...
	force_20pct = 0x0;
	if (current_value < CHG_CURRENT_OFF) {

		if (current_value * 5 < CHG_CURRENT_OFF)
			current_value = CHG_CURRENT_OFF;
		else
			current_value *= 5;
		force_20pct = 0x1;
	}

	bq24297_set_force_20pct(force_20pct);

	BUG_ON(current_value > MAX_CHG_CURRENT);
	BUG_ON(current_value < CHG_CURRENT_OFF);

	register_value = current_value - CHG_CURRENT_OFF;
	register_value >>= 6;
