...
static void mtkfb_fb_565_to_8888(struct fb_info *fb_info, bool fgFree)
{
	...
	if (fgFree)
		dmac_flush_range((void *)fb_info->screen_base,
			(void *)((unsigned int)fb_info->screen_base + fb_info->screen_size));
	else
...
static int mtkfb_probe(struct device *dev)
{
	...
	int r = 0;
	char *p = NULL;
	bool fgFreeReservedBuf = false;
	unsigned int buffer_size = 0;
#ifdef MTK_OVERLAY_ENGINE_SUPPORT
	const unsigned long wait_time = 500;
#else
	const unsigned long wait_time = msecs_to_jiffies(500);
#endif
	MSG_FUNC_ENTER();
	...
	mtkfb_fb_565_to_8888(fbi, fgFreeReservedBuf);

    /********************************************/

	if (fgFreeReservedBuf) {
	#ifdef MTK_OVERLAY_ENGINE_SUPPORT
		Disp_Ovl_Engine_Wait_Overlay_Complete(mtkfb_instance, wait_time);
	#else
		wait_event_interruptible_timeout(reg_update_wq, atomic_read(&OverlaySettingApplied)
						       && !atomic_read(&OverlaySettingDirtyFlag),
						       wait_time);
	#endif
		pr_err("mtkfb free reserve buffer, pa: 0x%x, size: 0x%x, mva: 0x%x\n",
			fbdev->fb_pa_base, buffer_size, fb_pa);
		r = disphal_free_reserve_buffer(fbdev->fb_pa_base, buffer_size, fb_pa);
		if (r != 0) {
			PRNERR("free_framebuffer failed\n");
			goto cleanup;
		}
	}
