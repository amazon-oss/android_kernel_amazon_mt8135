	...
	case DISP_OVL_ENGINE_IOCTL_SET_LAYER_INFO:
		{
			...
			if (hdmi_is_on == 1) {
				DISP_OVL_ENGINE_WARN("###dev.c layer=%d, en=%d, addr=0x%x, ion=0x%x, fmt=%d\n",
					laye_info.layer_id, laye_info.layer_enable,
					(unsigned int)laye_info.src_phy_addr,
					laye_info.ion_fd, laye_info.src_fmt);
				DISP_OVL_ENGINE_WARN("pitch=%d, src_w=%d, src_h=%d, dst_w=%d, dst_h=%d\n",
					laye_info.src_pitch, laye_info.src_width,
					laye_info.src_height, laye_info.tgt_width, laye_info.tgt_height);
			}
		}
		break;
