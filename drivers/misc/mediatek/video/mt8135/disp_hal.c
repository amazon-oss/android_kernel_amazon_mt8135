...
extern void DSCCHPAGE(UINT8 bVal);
...
int disphal_process_dbg_opt(char *buf, const char *opt)
{
	if (0 == strncmp(opt, "dbi:", 4)) {
		...
		} else if (0 == strncmp(opt + 4, "ch:", 3)) {
			char *p = (char *)opt + 7;
			//UINT8 dsc_cmd = 0; //skip cmd
			UINT8 bVal = (UINT8) simple_strtoul(p, &p, 16);
			DSCCHPAGE(bVal);
			sprintf(buf, "dsc_cmd: change page = 0x%02X\n", bVal);
...
int disphal_allocate_fb(struct resource *res, unsigned int *pa, unsigned int *va,
			unsigned int *dma_pa, bool fgFree)
{
	unsigned int addr;
...
int disphal_free_reserve_buffer(unsigned int buffer, unsigned int size, unsigned int mva)
{
	if (disp_use_mmu)
		m4u_dealloc_mva(M4U_CLNTMOD_LCDC_UI, buffer, size, mva);

	memblock_free_late(buffer, size);
	return 0;
}
