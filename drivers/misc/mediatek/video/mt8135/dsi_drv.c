...
#define NS_TO_CYCLE(n, c)	((n) / c + (((n) % c) ? 1 : 0))
	...
	if (system_type == BOARD_ID_MEMPHIS || system_type == BOARD_ID_THEBES)
		timcon0.HS_TRAIL = 0x0B; /* To pass MIPI spec on the memphis/thebes board. */
...
/* timcon1.TA_SACK         = (lcm_params->dsi.TA_SACK == 0) ? 1 : lcm_params->dsi.TA_SACK; */
	...
	timcon2.CLK_TRAIL =
	    ((lcm_params->dsi.CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64,
	     ...
	     CLK_TRAIL) + 0x0a;
	if (system_type == BOARD_ID_MEMPHIS || system_type == BOARD_ID_THEBES)
		timcon2.CLK_TRAIL = 0x05; /* To pass MIPI spec on the memphis/thebes board. */
