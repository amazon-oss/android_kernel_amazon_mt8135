							...
							u64 t, s16 *d)
{
	...
	u8 buf[IIO_BUFFER_BYTES];
	int i;
	static int debug_8bytedatacounter;
	if ((hdr & (~STEP_INDICATOR_MASK)) == ACCEL_HDR) {
		if (debug_8bytedatacounter > 300) {
			debug_8bytedatacounter = 0;
			pr_err("%s: Received Accel Sensor Data at Driver Layer %d, %d, %d\n",  __func__, d[0], d[1], d[2]);
		} else {
			debug_8bytedatacounter++;
		}
	}
...
irqreturn_t inv_read_fifo(int irq, void *dev_id)
{
	...
	struct inv_reg_map_s *reg;
	u64 pts1;
	static int debug_interruptcounter;

#define DMP_MIN_RUN_TIME (37 * NSEC_PER_MSEC)
	/* if (st->suspend_state)  JPL
		return IRQ_HANDLED; */
	down(&st->suspend_resume_lock);	
	mutex_lock(&indio_dev->mlock);
	if (st->chip_config.dmp_on) {
	...
	if (bpm) {
		...
		if (debug_interruptcounter > 50) {
			pr_err("%s: Received Sensor Interrupt to read FIFO and Fifo_count is %d\n", __func__, fifo_count);
			debug_interruptcounter = 0;
		} else {
			debug_interruptcounter++;
		}
