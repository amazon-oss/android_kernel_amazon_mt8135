...
static int inv_turn_on_engine(struct inv_mpu_state *st)
{
	...
	if (st->chip_config.gyro_enable | st->chip_config.accel_enable) {
		printk("inv_mpu_setup: Enable Gyro or Accel Sensor\n");
		...
		printk("inv_mpu_setup: Disable all Sensors\n");
		w = (BIT_PWR_GYRO_STBY |
				BIT_PWR_ACCEL_STBY |
				BIT_PWR_PRESSURE_STBY);
	}
	printk("inv_mpu_setup:Power Management 2 Reg = %d\n", w);
...
static int inv_get_accel_gyro_rate(int compass_rate)
{
	...
	i = 0;
	while ((i < (ARRAY_SIZE(accel_gyro_rate) - 1)) &&
