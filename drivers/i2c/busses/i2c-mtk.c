...
static int mt_deal_result(struct mt_i2c *i2c, struct mt_i2c_msg *msg)
{
	...
	if (!i2c->trans_data.trans_auxlen) {
		tmo = i2c->trans_data.trans_num * i2c->trans_data.trans_len;
	}
	else {
		/*
		 * This is for the combined msg.
                 *
		 * trans_len is for the 1st msg and
		 * trans_auxlen is for the 2nd one.
		 */
		tmo = i2c->trans_data.trans_len + i2c->trans_data.trans_auxlen;
	}
	...
	tmo = ((tmo * HZ * I2C_OVERHEAD_FACTOR) / 10) / i2c->sclk;
	tmo = tmo / 1000;
	tmo += (HZ / 20) ? : 1;	/* 50ms minimum timeout */
...
static int mt_i2c_resume(struct device *dev)
{
	...
	/* clear up i2c speed mode to allow speed to reset */
	i2c->mode = I2C_SPEED_MODE_SIZE;
