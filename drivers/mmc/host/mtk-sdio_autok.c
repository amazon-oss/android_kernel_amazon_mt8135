...
#define msdc_reset(id) \
	do { \
		...
		mb(); /* sequence access requirement */\
...
{
	if (volt != 1150000)
		pr_err("%s:: Can't change to this volt %u, please check!\n", __func__, volt);
}
#endif
 ...
 *			    Functions Implement				   *
 *****************************************************************************/
static DEFINE_SPINLOCK(SDIO_IRQ_lock);	/* maybe work abnormal */
				...
				unsigned long timeout)
{
	...
	if (opcode == MMC_SEND_OP_COND || opcode == SD_APP_OP_COND) {
		...
		dev_info(host->dev, "msdc%d select card<0x%.8x>", host->id, cmd->arg);	/* select and de-select */
	 ...
	 */
	rawcmd = opcode | msdc_rsp[resp] << 7 | ((data->blksz & 0xFFF) << 16);
	...
	/* pr_err("CMD<%d><0x%.8x> Arg<0x%.8x> %d blks",
	opcode , rawcmd, cmd->arg, (cmd->data==NULL)? 0 :cmd->data->blocks); */
	...
	if (opcode == MMC_SEND_STATUS) {
		for (;;) {
			...
			if (time_after(jiffies, tmo)) {
				pr_err("XXX cmd_busy timeout: before CMD<%d>", opcode);
		...
		for (;;) {
			if (!sdc_is_busy())
				break;
			if (time_after(jiffies, tmo)) {
				pr_err("XXX sdc_busy timeout: before CMD<%d>", opcode);
				cmd->error = (unsigned int)-ETIMEDOUT;
				msdc_reset_hw(host->id);
				return cmd->error;
			}
					      ...
					      int tune, unsigned long timeout)
{
	...
	while (1) {
		...
		if (time_after(jiffies, tmo)) {
			dev_err(host->dev, "XXX CMD<%d> polling_for_completion timeout ARG<0x%.8x>",
				cmd->opcode, cmd->arg);
			cmd->error = (unsigned int)-ETIMEDOUT;
			host->sw_timeout++;
	...
	if (intsts & cmdsts) {
		...
		if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY) ||
			...
			dev_err(host->dev, "XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>",
				cmd->opcode, cmd->arg);
			msdc_reset_hw(host->id);
		} else if (intsts & MSDC_INT_CMDTMO) {
			cmd->error = (unsigned int)-ETIMEDOUT;
			dev_err(host->dev, "XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>", cmd->opcode,
				cmd->arg);
			msdc_reset_hw(host->id);
		}
#ifdef MTK_MSDC_USE_CMD23
		if ((sbc != NULL) && (host->autocmd & MSDC_AUTOCMD23)) {
			if (intsts & MSDC_INT_ACMDRDY) {
				...
				*arsp = sdr_read32(SDC_ACMD_RESP);
				...
				dev_err(host->dev, "autocmd23 crc error\n");
				...
				dev_err(host->dev, "autocmd23 to error\n");
...
int autok_start_rw(struct msdc_host *host, u8 *value, unsigned size, unsigned blocks, bool write)
{
	...
	MSDC_ASSERT(host);
	MSDC_ASSERT(host->base);
	MSDC_ASSERT(host->data);
	MSDC_ASSERT(host->mrq);

	MSDC_ASSERT(host->mrq->data);
	MSDC_ASSERT(host->mrq->cmd);

	MSDC_ASSERT(host->mrq->cmd->data);
	MSDC_ASSERT(host->mrq->cmd->mrq);
	...
	if (msdc_txfifocnt() || msdc_rxfifocnt()) {
		dev_err(host->dev, "[%s][SD%d] register abnormal,please check!\n", __func__,
			host->id);
		msdc_reset_hw(host->id);
	}
	...
	events = write ? msdc_pio_write(host, data) : msdc_pio_read(host, data);
	cmd->error = (events | MSDC_INT_XFER_COMPL) ? 0 :
	    (events | MSDC_INT_DATCRCERR) ? (unsigned int)-EIO : (unsigned int)-ETIMEDOUT;
	/*msdc_data_xfer_done(host, events, host->mrq, data); */
...
{
	unsigned int vcore_uv = 1150000;
	unsigned int idx, size;
	unsigned int vcore_uv_offset;
...
#define MAX(a, b) ((a) > (b) ? (a) : (b))
	...
	vcore_uv_offset = size - 1;

	vcore_uv = MAX(vcore_uv, mt65x2_vcore_tbl[0]);
	vcore_uv = MIN(vcore_uv, mt65x2_vcore_tbl[size - 1]);

	for (idx = 0; idx < size; idx++) {
		if ((mt65x2_vcore_tbl[idx] <= vcore_uv)
			&& (vcore_uv < mt65x2_vcore_tbl[((idx + 1) == size) ? idx : idx + 1])) {
	...
	AUTOK_PRINT("[%s] Current Vcore = %u(%u:%u)\r\n", __func__, vcore_uv, vcore_uv_offset,
		    size);
		    ...
		    unsigned int u4Len, unsigned int u4Cmd)
{
	...
	MSDC_ASSERT(host);
	MSDC_ASSERT(pBuffer);
	MSDC_ASSERT((u4Cmd == CMD_53) || (u4Cmd == CMD_52));
	...
	if (u4Cmd == CMD_53) {
		...
		dev_err(host->dev, "[%s] Doesn't support u4Cmd = %d\n", __func__, u4Cmd);
		ret = -1;
	}
...
int msdc_autok_adjust_param(struct msdc_host *host, enum AUTOK_PARAM param, u32 *value, int rw)
{
	...
	case CLK_DRV:
		...
		if (host->id != 2) {
			dev_err(host->dev, "[%s] MSDC%d doesn't support AUTO K\n", __func__,
				host->id);
			return -1;
		}

		reg = (u32) (MSDC2_CLK_DRVING_BASE);
		field = (u32) (MSDC2_CLK_DRVING);
		break;
	case CMD_DRV:
		if ((rw == AUTOK_WRITE) && (*value > 7)) {
			dev_err(host->dev,
		...
		if (host->id != 2) {
			dev_err(host->dev, "[%s] MSDC%d doesn't support on AUTO K\n", __func__,
				host->id);
			return -1;
		}

		reg = (u32) (MSDC2_CMD_DRVING_BASE);
		field = (u32) (MSDC2_CMD_DRVING);
		break;
	case DAT_DRV:
		if ((rw == AUTOK_WRITE) && (*value > 7)) {
			dev_err(host->dev,
		...
		reg = (u32) (MSDC2_DAT_DRVING_BASE);
		field = (u32) (MSDC2_DAT_DRVING);
		break;
	...
	case PAD_CLK_TXDLY:
		...
		break;
	default:
		dev_err(host->dev, "[%s] Value of [enum AUTOK_PARAM param] is wrong\n", __func__);
		return -1;
	}
	...
	if (rw == AUTOK_READ)
		...
		dev_err(host->dev, "[%s] Value of [int rw] is wrong\n", __func__);
		return -1;
	}
...
static E_RESULT_TYPE autok_read_test(struct msdc_host *host)
{
	...
	for (i = 0; i < TUNING_DATA_NO; i++) {
		...
		if (memcmp
			...
			dev_err(host->dev, "[%s] E_RESULT_CMP_ERR\n", __func__);
			goto end;
		}
	}
end:
...
#ifdef AUTOK_DEBUG
			dev_err(host->dev, "write: 0x%x read: 0x%x\r\n", tuning_data[i],
	...
	for (bit = 0; bit < 32; bit++) {
		if (prAutok_raw_scan->RawData & (1 << bit)) {
			...
			case RD_SCAN_PAD_BOUND_E:
				if (filter) {
					/* (prAutok_raw_scan->Reg1Cnt <= AUTOK_TUNING_INACCURACY) && */
					if (((bit - prAutok_raw_scan->BoundReg1_E) <=
			...
			case RD_SCAN_PAD_BOUND_E_2:
				...
				if (filter) {
					...
					/* In case of the hole locates near region1 */
						...
						AUTOK_PRINT
						    ("[ERR] Find holes on raw data when CKGEN=%d,"
						     " but can NOT filter! \r\n",
						     prAutok_raw_scan->CurCKGEN);
			...
			while (x < SCALE_PAD_TUNE_DATRDDLY && reTuneCmd == 0) {
				...
				for (m = 0; m < AUTOK_RDAT_TIMES; m++) {
					...
					if (res != E_RESULT_PASS) {
						data.raw_data |= (1 << x);
						AUTOK_PRINT
						    ("[Debug]tune read : error,Error Code 0x%x===\r\n",
						     res);

						if (autok_recovery(host)) {
							AUTOK_PRINT
							    ("===tune read : error,"
							    " fail to bring to tranfer status===\r\n");
							goto exit;
						}
						if (res == E_RESULT_CMD_CRC) {
							AUTOK_PRINT
							    ("[WARN] CMD CRC error in tuning read[%d %d],"
							     " need to tune command again!!\r\n",
							     x, m);
							reTuneCmd = 1;
						}
						break;
	...
	while (k <= range_max) {
		...
		if (k && !fCMDIntDlyFound && !fCMDMatrixStress) {
			if (ABS_DIFF(sel, autok_ckg_data[k - 1].interDelaySel) > 8) {
				...
				AUTOK_PRINT
				    ("[WARN]Restart to scan CKGEN "
				     "due to invalid CMD matrix!IntDly Gap(%d)\r\n",
		...
		if (max_score == 0) {
			AUTOK_PRINT
			    ("autok algorithm for tuning cmd "
			    "internal delay need to scan more!!\r\n");
		...
		if (!fCMDIntDlyFound) {
			...
			/*
			   autok_select_range(autok_cmd_ckgdly[k].raw_data,
					&pAutoKData[E_MSDC_PAD_TUNE_CMDRDLY].data.sel);
...
ReTuneCMDPad:
	...
	if (stg == TUNING_STG1) {
		 ...
		 */
			...
			AUTOK_PRINT
			    ("[WARN] Can NOT find the best CMD pad delay,"
			    " the timing between CMD & RDAT differs too much!\r\n");
			/*
		...
		if (!FBoundCnt) {
			...
			if (freq_mhz <= 100) {
				...
				if (pPadDlyRes->fFBoundRefCMD) {
					...
					AUTOK_PRINT
					    ("Refered from CMD: FBoundCKGEN:%d, "
					     "FBoundCnt:%d, FBoundLMargin:%d, IntBound:%d\r\n",
...
ChngBaseCK:
		...
		/* In case of Right boundary at CKGEN = 0 */
			...
			if (baseCKGEN > 2)
				RBoundPad -=
				    baseCKGEN * MIN_CLK_GEN_DELAY_IN_PS / MIN_PAD_DELAY_IN_PS;
			else
				RBoundPad -=
				    baseCKGEN * DIV_CEIL_FUNC(MIN_CLK_GEN_DELAY_IN_PS,
							      MIN_PAD_DELAY_IN_PS);

			AUTOK_PRINT
			    ("Right boundary found at initial timing(CKGEN=%d)(Rbound:%d vs. Margin.Max:%d)\r\n",
		...
		/* In case of right boundary by shifting CKGEN */
			...
			if (autok_rdata_scan[RealCKGEN].BoundReg1_S && fLeftBoundary) {
				...
				autok_select_range(autok_rdata_scan[RealCKGEN].RawData,
						   &RealPadDelay);
			}
			/*
		...
		while (x < SCALE_PAD_TUNE_DATWRDLY && reTuneCmd == 0) {
			...
			for (m = 0; m < (AUTOK_WDAT_TIMES); m++) {
				...
				if (res != E_RESULT_PASS) {
					...
					if (res == E_RESULT_CMD_CRC) {
						AUTOK_PRINT
						    ("[WARN] CMD CRC error in tuning write[%d %d],"
						     " need to tune command again!!\r\n",
						     x, m);
						reTuneCmd = 1;
					}
		...
		if (vcore_uv <= g_autok_vcore_sel[0])
			...
			for (idx = 0; idx < AUTOK_VCORE_SCAN_NUM; idx++) {
				if ((vcore_uv > autok_vcore_sel[idx]) &&
					(vcore_uv <= autok_vcore_sel[((idx + 1) == AUTOK_VCORE_SCAN_NUM)
					? idx : idx + 1])) {
