...
enum AUTOK_PARAM {
	...
	/* DAT0 Pad RX Delay Line Control (for MSDC RD), Total 32 stages */
	DAT0_RD_DLY,

	/* DAT1 Pad RX Delay Line Control (for MSDC RD), Total 32 stages */
	DAT1_RD_DLY,

	/* DAT2 Pad RX Delay Line Control (for MSDC RD), Total 32 stages */
	DAT2_RD_DLY,

	/* DAT3 Pad RX Delay Line Control (for MSDC RD), Total 32 stages */
	DAT3_RD_DLY,

	/* Write Data Status Internal Delay Line Control. This register is used
	   to fine-tune write status phase latched by MSDC internal clock. Total 32 stages */
	DAT_WRD_DLY,

	/* Rx  Delay Line Control. Total 32 stages */
	DAT_RD_DLY,

	/* CMD Response Internal Delay Line Control. This register is used to fine-tune
	   response phase  latched by MSDC internal clock. Total 32 stages */
	CMD_RESP_RD_DLY,

	/* CMD Pad RX Delay Line Control. This register is used to fine-tune
	   CMD pad macro respose latch timing. Total 32 stages */
	CMD_RD_DLY,
	...
	/* Data line delay line fine tune selection. 1'b0: All data line share one delay
	   ...
	   1'b1: Each data line has its own delay selection value indicated
	   by Data line (x): DAT_RD_DLY(x).DAT0_RD_DLY */
	DATA_DLYLINE_SEL,
...
char *reset_autok_cursor(int voltage);
/* void clear_autok_buf(); */
...
#ifndef LOCAL_VAR_DESC
extern char *log_info;
extern int total_msg_size;
extern struct msdc_host *mtk_msdc_host[HOST_MAX_NUM + 1];
#endif
