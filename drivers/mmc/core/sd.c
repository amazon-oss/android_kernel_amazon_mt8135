...
#define VENDOR_LEXAR	0x28
#define OEMID_LEXAR		0x4245
#define NAME_LEXAR		"     "
#define U3_LEXAR		0x6
	/* Limit some Lexar SD cards to SDR25 */
	if (card->cid.manfid == VENDOR_LEXAR && card->cid.oemid == OEMID_LEXAR &&
		timing == U3_LEXAR && !strncmp(NAME_LEXAR, card->cid.prod_name, 5) &&
	    card->sw_caps.uhs_max_dtr > UHS_SDR25_MAX_DTR) {
		timing = MMC_TIMING_UHS_SDR25;
		card->sw_caps.uhs_max_dtr = UHS_SDR25_MAX_DTR;
	}
...
int mmc_sd_get_csd(struct mmc_host *host, struct mmc_card *card)
{
	...
	dev_err(host->parent, "sdcard csd %08x%08x%08x%08x\n", card->raw_csd[0], card->raw_csd[1],
		card->raw_csd[2], card->raw_csd[3]);
	...
	struct mmc_card *oldcard)
{
	...
	/* Initialization sequence for UHS-I cards and soft-reseted cards */
	if ((rocr & SD_ROCR_S18A) || (oldcard != NULL && (old_rocr & SD_ROCR_S18A))) {
