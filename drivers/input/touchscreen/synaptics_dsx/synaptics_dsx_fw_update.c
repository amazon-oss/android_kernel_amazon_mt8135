...
DECLARE_COMPLETION(fwu_remove_complete2);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		retval = snprintf(fwu->image_name, MAX_IMAGE_NAME_LEN,
				"synaptics/board_%x_startup_fw_update.img", system_type);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
int synaptics_fw_updater2(const unsigned char *fw_data)
{
	int retval;
...
EXPORT_SYMBOL(synaptics_fw_updater2);

#ifdef DO_STARTUP_FW_UPDATE
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	synaptics_fw_updater2(NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_fw_updater2(fwu->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
...
{
	synaptics_rmi4_new_function2(&fwu_module, true);
...
{
	synaptics_rmi4_new_function2(&fwu_module, false);

	wait_for_completion(&fwu_remove_complete2);
