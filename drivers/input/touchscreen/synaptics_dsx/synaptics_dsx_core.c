...
static void synaptics_rmi4_f01_hwreset(struct synaptics_rmi4_data *rmi4_data);

static ssize_t synaptics_rmi4_f01_reset_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_f01_hwreset_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(hwreset, S_IWUGO,
			synaptics_rmi4_show_error,
			synaptics_rmi4_f01_hwreset_store),
	...
	__ATTR(esd_reset, S_IRUGO,
			synaptics_esd_reset_show,
			synaptics_rmi4_store_error),
};
...
static void synaptics_watchdog_work(struct work_struct *work)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f01_query_base_addr, &data, sizeof(data));
	if (retval >= 0)
		dev_dbg(rmi4_data->pdev->dev.parent,
			"data=0x%x\n", data);
	...
	if (unlikely(retval < 0 || data == 0x0)) {
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: Failed read ID for watchdog, reset touch\n",
				__func__);
		synaptics_rmi4_f01_hwreset(rmi4_data);
		watchdog_reset++;
	}
...
static void synaptics_rmi4_f01_hwreset(struct synaptics_rmi4_data *rmi4_data)
{
	...
	printk("%s: reset_gpio=%d, reset_on=%d\n", __func__, bdata->reset_gpio, reset_on);
	if (bdata->reset_gpio >= 0) {
		...
		hwPowerOn(MT65XX_POWER_LDO_VGP6, VOL_3300, "touch");
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (extra_data->data15_size) {
		...
		index = min(extra_data->data15_size - 1,
			(unsigned char)sizeof(extra_data->data15_data) - 1); /* Highest byte */
...
void synaptics_rmi4_new_function2(struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
EXPORT_SYMBOL(synaptics_rmi4_new_function2);
