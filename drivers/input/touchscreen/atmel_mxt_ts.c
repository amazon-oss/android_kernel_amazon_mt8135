...
#define MXT_1664_1066_FAMILY_ID	164
#define MXT_1066_VARIANT_ID	11
/* FIXME - disable firmware update before all mxt touch tested */
...
#define MXT_1664_FW_VERSION_MAJOR	1
#define MXT_1664_FW_VERSION_MINOR	0
#define MXT_1664_FW_BUILD	0xAA
#define MXT_1066_FW_VERSION_MAJOR	1
#define MXT_1066_FW_VERSION_MINOR	2
#define MXT_1066_FW_BUILD	0xC2
#define FW_VERSION_MATCHED	0xAA
#endif
/* Timeout for workdog work in ms  */
#define MXT_WATCHDOG_TIMEOUT 5000
static int watchdog_reset;
static ssize_t mxt_esd_reset_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t mxt_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
struct mxt_data {
	...
	struct workqueue_struct *poweron_workqueue;
...
#endif
#define MXT_1664_BOOTLOADER_ID	97
#define MXT_1664_BOOTLOADER_VERSION	2
/* FIXME: To be corrected for 1066 */
#define MXT_1066_BOOTLOADER_ID	1
#define MXT_1066_BOOTLOADER_VERSION	0
...
static void mxt_start_wd_work(struct mxt_data *data);
static ssize_t mxt_hwreset_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static int mxt_debug_msg_init(struct mxt_data *data)
{
	...
	data->debug_msg_attr.attr.mode = 0600;
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	dev_dbg(&data->client->dev, "touch irq number %d\n", irq_count++);
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	return;
	/* FIXME: disable regulator currently */
	/*
...
static int mxt_initialize_t100_input_device(struct mxt_data *data)
{
	...
	/* Disable ABS_MT_TOOL_TYPE as current touch panel doesn't support stylus */
...
static void mxt_config_cb(const struct firmware *cfg, void *ctx)
{
	...
	/* Start watchdog work */
	if (data->pdata->esd_auto_recover)
		mxt_start_wd_work(data);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	if (error) {
		if (error == FW_VERSION_MATCHED)
			dev_info(&data->client->dev, "No update for the firmware\n");
		else {
			dev_err(&data->client->dev, "The firmware update failed(%d)\n", error);
			data->pdata->rst_power();
			data->in_bootloader = false;
			data->suspended = true;
			mxt_initialize(data);
		}
		...
		if (error)
			dev_err(&data->client->dev, "Atmel mxt initialize failed(%d)\n", error);
	}

}

static int mxt_check_firmware_version(struct device *dev)
{
	struct mxt_data *data = dev_get_drvdata(dev);

	/* May due to app crc error, so update the firmware anyway */
	if (data->info == NULL)
		return 0;
	/* Firmware update */
	if (data->info->family_id == MXT_1664_1066_FAMILY_ID) {
		if (data->info->version >> 4 == MXT_1664_FW_VERSION_MAJOR &&
			(data->info->version & 0xf) == MXT_1664_FW_VERSION_MINOR &&
			data->info->build == MXT_1664_FW_BUILD) {
			dev_info(dev, "Firmware version is the same\n");
			return FW_VERSION_MATCHED;
		}

	} else {
		if (data->info->version >> 4 == MXT_1066_FW_VERSION_MAJOR &&
			(data->info->version & 0xf) == MXT_1066_FW_VERSION_MINOR &&
			data->info->build == MXT_1066_FW_BUILD) {
			dev_info(dev, "Firmware version is the same\n");
			return FW_VERSION_MATCHED;
		}
	}
...
static int mxt_initialize(struct mxt_data *data)
{
	...
	while (1) {
		...
		if (++recovery_attempts > 1) {
			...
			if ((NULL != data->pdata->fw_name) && (data->fw_name == NULL)) {
				int fw_len = strlen(data->pdata->fw_name) + strlen("_mxt_****.fw") + 1;
				data->fw_name = kmalloc(fw_len, GFP_KERNEL);
				if (NULL == data->fw_name) {
					error = -ENOMEM;
					goto err_free_object_table;
				}
				strlcpy(data->fw_name, data->pdata->fw_name, fw_len);
				if ((buf[1] == MXT_1664_BOOTLOADER_ID) &&
					(buf[2] == MXT_1664_BOOTLOADER_VERSION))
					strlcat(data->fw_name, "_mxt_1664.fw", fw_len);
				else if ((buf[1] == MXT_1066_BOOTLOADER_ID) &&
					(buf[2] == MXT_1066_BOOTLOADER_VERSION))
					strlcat(data->fw_name, "_mxt_1066.fw", fw_len);
				else
					dev_err(&client->dev, "Not found atmel 1664/1066\n");
			}
#ifdef DO_STARTUP_FW_UPDATE
			if (data->fwu_workqueue == NULL) {
				data->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue");
				INIT_DELAYED_WORK(&data->fwu_work, fwu_startup_fw_update_work);
			}
			if (data->fw_name)
				queue_delayed_work(data->fwu_workqueue, &data->fwu_work,
	...
	/* Firmware update */
	if ((NULL != data->pdata->fw_name) && (data->fw_name == NULL)
		&& (data->info->family_id == MXT_1664_1066_FAMILY_ID)) {
		int fw_len = strlen(data->pdata->fw_name) + strlen("_mxt_****.fw") + 1;
		data->fw_name = kmalloc(fw_len, GFP_KERNEL);
		if (NULL == data->fw_name) {
			error = -ENOMEM;
		...
		strlcpy(data->fw_name, data->pdata->fw_name, fw_len);
		if (data->info->variant_id == MXT_1066_VARIANT_ID)
			strlcat(data->fw_name, "_mxt_1066.fw", fw_len);
		else
			strlcat(data->fw_name, "_mxt_1664.fw", fw_len);
	}
	if (
#ifdef CONFIG_IDME
			(idme_get_bootmode() != BOOT_MODE_DIAG) &&
#endif
			(mxt_check_firmware_version(&data->client->dev) == 0)) {
		if (data->fwu_workqueue == NULL) {
			data->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue");
			INIT_DELAYED_WORK(&data->fwu_work, fwu_startup_fw_update_work);
		}
		...
		dev_info(&data->client->dev, "%s:Skipping Touch FW update!!\n", __func__);
#endif
	...
	if (data->info->family_id == MXT_1664_1066_FAMILY_ID) {
		if (data->info->variant_id == MXT_1066_VARIANT_ID) {
			if ((NULL != data->pdata->cfg_name) && (data->cfg_name == NULL)) {
				int cfg_len = strlen(data->pdata->cfg_name) + strlen("_mxt_****.raw") + 1;
				data->cfg_name = kmalloc(cfg_len, GFP_KERNEL);
				if (NULL == data->cfg_name) {
					error = -ENOMEM;
					goto err_free_object_table;
				}
				strlcpy(data->cfg_name, data->pdata->cfg_name, cfg_len);
				strlcat(data->cfg_name, "_mxt_1066.raw", cfg_len);
			}
		} else {
			if ((NULL != data->pdata->cfg_name) && (data->cfg_name == NULL)) {
				int cfg_len = strlen(data->pdata->cfg_name) + strlen("_mxt_****.raw") + 1;
				...
				strlcpy(data->cfg_name, data->pdata->cfg_name, cfg_len);
				strlcat(data->cfg_name, "_mxt_1664.raw", cfg_len);
			}
		}
	}
	if (data->cfg_name) {
...
static int mxt_set_t7_power_cfg(struct mxt_data *data, u8 sleep)
{
	...
	error = __mxt_read_reg(data->client, data->T7_address,
				sizeof(data->t7_cfg), &deepsleep);
	if (error)
		return error;
	dev_info(dev, "Get T7 ACTV:%d IDLE:%d\n",
		deepsleep.active, deepsleep.idle);
...
#ifdef DO_STARTUP_FW_UPDATE
	ret = mxt_check_firmware_version(dev);
	/* ret: 0 - firmware need to update */
	if (ret)
		return ret;
#endif
...
static DEVICE_ATTR(suspend, S_IWUSR, NULL, mxt_suspend_store);
static DEVICE_ATTR(esd_reset, S_IRUSR, mxt_esd_reset_show, NULL);
static DEVICE_ATTR(idshow, S_IRUSR, mxt_id_show, NULL);
static DEVICE_ATTR(hw_reset, S_IWUSR, NULL, mxt_hwreset_store);
...
static struct attribute *mxt_attrs[] = {
	...
	&dev_attr_esd_reset.attr,
	&dev_attr_idshow.attr,
...
static ssize_t mxt_esd_reset_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "watchdog_rst=%d\n", watchdog_reset);
}
...
static void mxt_start_wd_work(struct mxt_data *data)
{
	queue_delayed_work(data->wd_workqueue,
			&data->wd_work,
			msecs_to_jiffies(MXT_WATCHDOG_TIMEOUT));
}

static void mxt_poweron_work(struct work_struct *work)
{
	struct mxt_data *data =
			container_of(work, struct mxt_data,
			poweron_work.work);
	struct input_dev *input_dev = data->input_dev;
	if (!data->enabled || !data->suspended ||
		!input_dev->users || data->in_bootloader)
		return;
...
static void mxt_watchdog_work(struct work_struct *work)
{
	...
	retval = __mxt_read_reg(data->client, 0, sizeof(value), &value);

	if (retval >= 0)
		dev_dbg(&data->client->dev,
			"data=0x%x\n", value);

	if (unlikely(retval < 0 || value == 0x0)) {
		dev_err(&data->client->dev,
...
static int mxt_gpio_init(int irq, int reset)
{
	int error;

	error = mt_pin_set_pull(irq, MT_PIN_PULL_ENABLE_UP);
	if (error)
		return error;
	/* FIXME: reset pin to be moved here instead of initization in board-xxx.c */
	error = mt_pin_set_mode_eint(irq);
	if (error)
		return error;
	error = gpio_request(irq, "mxt_irq");
	if (error)
		return error;
	error = gpio_direction_input(irq);
	if (error)
		gpio_free(irq);
	return error;
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	error = mxt_gpio_init(client->irq, (int)pdata->gpio_reset);
	if (error) {
	...
	data->wd_workqueue =
			create_singlethread_workqueue("mxt_wd_workqueue");
	INIT_DELAYED_WORK(&data->wd_work,
			mxt_watchdog_work);

	data->poweron_workqueue =
			create_singlethread_workqueue("mxt_poweron_workqueue");
	INIT_DELAYED_WORK(&data->poweron_work,
			mxt_poweron_work);
...
static int mxt_remove(struct i2c_client *client)
{
	...
	if (data->pdata->esd_auto_recover)
		cancel_delayed_work_sync(&data->wd_work);

	flush_workqueue(data->wd_workqueue);
	destroy_workqueue(data->wd_workqueue);
	cancel_delayed_work_sync(&data->poweron_work);
	flush_workqueue(data->poweron_workqueue);
	destroy_workqueue(data->poweron_workqueue);
...
static void mxt_early_suspend(struct early_suspend *es)
{
	struct mxt_data *data = container_of(es, struct mxt_data, early_suspend);
	struct input_dev *input_dev = data->input_dev;
...
static void mxt_late_resume(struct early_suspend *es)
{
	...
	queue_delayed_work(data->poweron_workqueue,
				&data->poweron_work,
				msecs_to_jiffies(0));
