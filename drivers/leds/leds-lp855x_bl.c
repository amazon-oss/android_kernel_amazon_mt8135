			...
			enum led_brightness brightness)
{
	...
	if (mode == PWM_BASED) {
		...
		u8 val = brightness;
		if (lp->pdata->brightness_limit)
			val = brightness * lp->pdata->brightness_limit /
				lp->pdata->max_brightness;
		...
		enum led_brightness brightness)
{
	...
	state = pdata->cool_dev.state;

	/* Record the latest value from userspace */
	pdata->cool_dev.level = brightness;

	if (state) {
		if (brightness > pdata->cool_dev.levels[state - 1]) {
			/*
			 ...
			 */
			led_cdev->brightness = pdata->cool_dev.levels[state - 1];
			goto out;
		}
	}
			   ...
			   unsigned long *state)
{
	...
	struct lp855x_platform_data *pdata = lp->pdata;
	*state = pdata->cool_dev.max_state;
	return 0;
}
			   ...
			   unsigned long state)
{
	...
	if (!pdata->cool_dev.state)
		pdata->cool_dev.level = lp->last_brightness;

	if (pdata->cool_dev.state == state)
		goto out;

	max_state = pdata->cool_dev.max_state;
	pdata->cool_dev.state = (state > max_state) ? max_state : state;

	if (!pdata->cool_dev.state)
		level = pdata->cool_dev.level;
	else {
		level = pdata->cool_dev.levels[pdata->cool_dev.state - 1];
		if (level > pdata->cool_dev.level)
			goto out;
	}
...
static ssize_t levels_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	struct lp855x *lp = cdev->devdata;
	struct lp855x_platform_data *pdata = lp->pdata;
	struct mtk_cooler_platform_data *cool_dev = &(pdata->cool_dev);
	int i;
	int offset = 0;
...
static int lp855x_probe(struct i2c_client *cl, const struct i2c_device_id *id)
{
	...
	pdata->cool_dev.cdev = thermal_cooling_device_register(pdata->cool_dev.type,
							     (void *)lp,
							     &cooling_ops);
	if (!pdata->cool_dev.cdev)
		return -EINVAL;
	device_create_file(&pdata->cool_dev.cdev->device, &dev_attr_levels);
