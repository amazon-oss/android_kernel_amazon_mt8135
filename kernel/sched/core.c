...
void __kprobes add_preempt_count(int val)
{
	...
	if (unlikely(__raw_get_cpu_var(mtsched_mon_enabled) & 0x1))
		MT_trace_preempt_off(CALLER_ADDR0, get_parent_ip(CALLER_ADDR1));
#endif
    }
}
...
#ifdef CONFIG_PREEMPT_MONITOR
	if (unlikely(__raw_get_cpu_var(mtsched_mon_enabled) & 0x1))
		MT_trace_preempt_on(CALLER_ADDR0, get_parent_ip(CALLER_ADDR1));
#endif
    }
	preempt_count() -= val;
}
...
int set_cpus_allowed_ptr(struct task_struct *p, const struct cpumask *new_mask)
{
	...
	if (!cpumask_intersects(new_mask, cpu_active_mask)) {
		ret = -EINVAL;
		printk_deferred(KERN_ERR "SCHED: intersects new_mask: %lu, cpu_active_mask: %lu\n", new_mask->bits[0], cpu_active_mask->bits[0]);
		goto out;
	}
