	...
	if (check_pack_buddy(cpu, p) && PA_ENABLE) {
		...
		if(PA_MON_ENABLE) {
			if(strcmp(p->comm, PA_MON) == 0 && cpu != per_cpu(sd_pack_buddy, cpu)) {
				printk_deferred(KERN_EMERG "[PA] %s PACK From CPU%d to CPU%d\n",
							p->comm, cpu, per_cpu(sd_pack_buddy, cpu));
				printk_deferred(KERN_EMERG "[PA]   Buddy RQ Usage = %u, Period = %u, NR = %u\n",
				...
				printk_deferred(KERN_EMERG "[PA]   Task Usage = %u, Period = %u\n",
  ...
  int target_cpu;
	...
	if(PA_MON_ENABLE) {
		if(strcmp(p->comm, PA_MON) == 0 && cpu != new_cpu) {
			printk_deferred(KERN_EMERG "[PA] %s Select From CPU%d to CPU%d\n", p->comm, cpu, new_cpu);
		}
	}
#endif /* CONFIG_HMP_POWER_AWARE_CONTROLLER */
			...
			if(PA_MON_ENABLE) {
				...
				for(i=0;i<4; i++) {
					if(strcmp(p->comm, &PA_MON[i][0]) == 0) {
						...
						printk_deferred(KERN_EMERG "[PA] %s PACK to CPU%d(CPU%d's buddy), pre(cpu%d)\n", p->comm, buddy_cpu, cpu, prev_cpu);
						printk_deferred(KERN_EMERG "[PA]   Buddy RQ Usage = %u, Period = %u, NR = %u\n",
...
static void move_task(struct task_struct *p, struct lb_env *env)
{
	...
	if(PA_MON_ENABLE) {
		if(strcmp(p->comm, PA_MON) == 0) {
			printk_deferred(KERN_EMERG "[PA] %s Balance From CPU%d to CPU%d\n",
		...
		if (cpumask_test_cpu(src_cpu, &buddy_cpu_map) &&
			...
			for(i=0;i<4;i++) {
				if(PA_MON_ENABLE && (strcmp(p->comm, &PA_MON[i][0]) == 0)) {
					printk_deferred(KERN_EMERG "[PA] %s Lazy balance from CPU%d to CPU%d\n",
