...
void unthrottle_offline_rt_rqs(struct rq *rq) {
	...
	for_each_leaf_rt_rq(rt_rq, rq) {
		...
		if (rt_rq_throttled(rt_rq)){
			rt_rq->rt_throttled = 0;
			printk_deferred(KERN_ERR "sched: RT throttling inactivated\n");
		}
	}
}
...
static int do_balance_runtime(struct rt_rq *rt_rq)
{
	...
	printk_deferred(KERN_EMERG" do_balance_runtime curr_cpu=%d, dst_cpu=%d, span=%lu\n",
	...
	for_each_cpu(i, rd->span) {
...
#ifdef MTK_DEBUG_CGROUP
		printk_deferred(KERN_EMERG" do_balance_runtime get lock cpu=%d\n", i);
#endif
		if(!raw_spin_trylock(&iter->rt_runtime_lock)){
#ifdef MTK_DEBUG_CGROUP
			printk_deferred(KERN_EMERG" do_balance_runtime try lock fail cpu=%d\n", i);
#endif
			continue;
		}
		/*
...
#ifdef MTK_DEBUG_CGROUP
		printk_deferred(KERN_EMERG"borrow, dst_cpu=%d, src_cpu=%d, src_cpu2=%d,src_addr=%x, dst_addr=%x,dst->rt_runtime=%llu, src->rt_runtime=%llu, diff=%lld, span=%lu\n",
		...
		if (diff > 0) {
			...
			more = 1;
#ifdef MTK_DEBUG_CGROUP
			printk_deferred(KERN_EMERG"borrow successfully, dst_cpu=%d, src_cpu=%d, src_cpu2=%d, src_addr=%x, dst_addr=%x,dst->rt_runtime=%llu, src->rt_runtime=%llu, diff=%lld, span=%lu\n",
				rt_rq->rq->cpu, i, iter->rq->cpu, iter, 
				rt_rq, rt_rq->rt_runtime, iter->rt_runtime, diff, rd->span->bits[0]);
#endif
...
static void __disable_runtime(struct rq *rq)
{
	...
	for_each_rt_rq(rt_rq, iter, rq) {
		 ...
		 */
#ifdef MTK_DEBUG_CGROUP
		printk_deferred(KERN_EMER"0. disable_runtime, cpu=%d, rd->span=%lu, rt_rq_addr=%x, rt_rq->rt_runtime=%llu, rt_b->rt_runtime=%llu\n",
		...
		for_each_cpu(i, rd->span) {
...
#ifdef MTK_DEBUG_CGROUP
			printk_deferred(KERN_EMERG"0. disable_runtime, cpu=%d,rt_b->rt_runtime=%llu, rt_rq->rt_runtime=%llu, want=%lld, rd->span=%lu\n",
...
#ifdef MTK_DEBUG_CGROUP
				printk_deferred(KERN_EMERG"1. disable_runtime, cpu=%d, %llu\n",
					i, iter->rt_runtime);
#endif
				continue;
			}
			...
			raw_spin_lock(&iter->rt_runtime_lock);
#ifdef MTK_DEBUG_CGROUP
			printk_deferred(KERN_EMERG"2-1. disable_runtime cpu=%d, want=%lld, iter->rt_runtime=%llu\n",
			...
			if (want > 0) {
				...
				want -= diff;
#ifdef MTK_DEBUG_CGROUP
				printk_deferred(KERN_EMERG"2. disable_runtime, rt_runtime=%llu, diff=%lld, want=%lld\n",
				...
				want -= want;
#ifdef MTK_DEBUG_CGROUP
				printk_deferred(KERN_EMERG"3. disable_runtime, rt_runtime=%llu, want=%lld\n",
										iter->rt_runtime, want);
#endif
			}
			raw_spin_unlock(&iter->rt_runtime_lock);
		...
		if(want){
#ifdef MTK_DEBUG_CGROUP
			printk_deferred(KERN_EMERG"4. disable_runtime, want=%lld, rt_rq->rt_runtime=%llu\n",
				want, rt_rq->rt_runtime);
			{
			struct rt_rq *iter = sched_rt_period_rt_rq(rt_b, 0);
			printk_deferred(KERN_EMERG"4-0. disable_runtime %llu\n", iter->rt_runtime);
			iter = sched_rt_period_rt_rq(rt_b, 1);
			printk_deferred(KERN_EMERG"4-1. disable_runtime %llu\n", iter->rt_runtime);
			iter = sched_rt_period_rt_rq(rt_b, 2);
			printk_deferred(KERN_EMERG"4-2. disable_runtime %llu\n", iter->rt_runtime);
			iter = sched_rt_period_rt_rq(rt_b, 3);
			printk_deferred(KERN_EMERG"4-3. disable_runtime %llu\n", iter->rt_runtime);
			}
#endif
...
balanced:
		...
		{
		struct rt_rq *iter = sched_rt_period_rt_rq(rt_b, 0);
		printk_deferred(KERN_EMERG"5-0. disable_runtime %llu\n", iter->rt_runtime);
		iter = sched_rt_period_rt_rq(rt_b, 1);
		printk_deferred(KERN_EMERG"5-1. disable_runtime %llu\n", iter->rt_runtime);
		iter = sched_rt_period_rt_rq(rt_b, 2);
		printk_deferred(KERN_EMERG"5-2. disable_runtime %llu\n", iter->rt_runtime);
		iter = sched_rt_period_rt_rq(rt_b, 3);
		printk_deferred(KERN_EMERG"5-3. disable_runtime %llu\n", iter->rt_runtime);
		}
#endif
		raw_spin_unlock(&rt_rq->rt_runtime_lock);
		raw_spin_unlock(&rt_b->rt_runtime_lock);
#ifdef MTK_DEBUG_CGROUP
		printk_deferred(KERN_ERR "disable_runtime after: rt_rq->rt_runtime=%llu rq_rt->rt_throttled=%d\n",
...
static void __enable_runtime(struct rq *rq)
{
	...
	for_each_rt_rq(rt_rq, iter, rq) {
		...
		if (rt_rq->rt_disable_borrow ){
			#ifdef MTK_DEBUG_CGROUP
			printk_deferred(KERN_EMERG"enable_runtime %d\n", rq->cpu);
			#endif
			rt_rq->rt_runtime = rt_b->rt_runtime;
			rt_rq->rt_time = 0;
			rt_rq->rt_throttled = 0;
...
#ifdef MTK_DEBUG_CGROUP
	printk_deferred(KERN_EMERG"do_sched_rt_period_timer curr_cpu=%d\n", smp_processor_id());
#endif
	for_each_cpu(i, span) {
		int enqueue = 0;
		...
		if( test_bit(idx, array->bitmap)){
			list_for_each_entry(rt_se, array->queue + idx, run_list){
				...
				if( (p->rt_priority == prio) && (0 == strncmp(p->comm, "wdtk", 4)) ){
					...
					printk_deferred(KERN_WARNING "sched: unthrottle %s\n", p->comm);
					return p;
				}
			}
		}
