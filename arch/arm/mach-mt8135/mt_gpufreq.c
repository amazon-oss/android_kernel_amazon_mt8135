...
****************************/
/* HACK: Remove*/
#ifdef abc123_DEVICE_TYPE
/* HACK: Remove once abc123 devices are available */
static struct mt_gpufreq_power_info mt_gpufreqs_golden_power[] = {
	{.gpufreq_khz = GPU_DVFS_F3, .gpufreq_power = 1050},
	{.gpufreq_khz = GPU_DVFS_F3, .gpufreq_power = 1000},
	{.gpufreq_khz = GPU_DVFS_F3, .gpufreq_power = 950},
	{.gpufreq_khz = GPU_DVFS_F3, .gpufreq_power = 900},
	{.gpufreq_khz = GPU_DVFS_F3, .gpufreq_power = 800},
	{.gpufreq_khz = GPU_DVFS_F3, .gpufreq_power = 650},
...
#ifdef abc123_DEVICE_TYPE
/* HACK: Remove once abc123 devices are available */
static unsigned int g_gpu_max_freq = GPU_DVFS_F3;
#else
...
{
	pr_debug("mt_gpufreq_check_freq_and_set_pll, g_cur_freq = %d\n", g_cur_gpu_freq);
...
static void mt_gpufreq_read_current_clock_from_reg(void)
{
	...
	pr_debug("mt_gpufreq_read_current_clock_from_reg: freq(%d)\n", freq);
}
...
static void mt_gpufreq_sel(unsigned int src, unsigned int sel)
{
	...
	if (src == GPU_FMFG) {
		...
		case GPU_CKSQ_MUX_CK:
			...
			pr_debug("mt_gpufreq_sel - GPU_CKSQ_MUX_CK\n");
			break;
		case GPU_UNIVPLL1_D4:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_UNIVPLL1_D4\n");
			break;
		case GPU_SYSPLL_D2:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_SYSPLL_D2\n");
			break;
		case GPU_SYSPLL_D2P5:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_SYSPLL_D2P5\n");
			break;
		case GPU_SYSPLL_D3:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_SYSPLL_D3\n");
			break;
		case GPU_UNIVPLL_D5:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_UNIVPLL_D5\n");
			break;
		case GPU_UNIVPLL1_D2:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_UNIVPLL1_D2\n");
			break;
		case GPU_MMPLL_D2:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MMPLL_D2\n");
			break;
		case GPU_MMPLL_D3:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MMPLL_D3\n");
			break;
		case GPU_MMPLL_D4:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MMPLL_D4\n");
			break;
		case GPU_MMPLL_D5:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MMPLL_D5\n");
			break;
		case GPU_MMPLL_D6:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MMPLL_D6\n");
			break;
		case GPU_MMPLL_D7:
			clk_cfg = (clk_cfg & 0xFFF0FFFF) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_0, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MMPLL_D7\n");
			break;
		default:
			pr_err("mt_gpufreq_sel - NOT support PLL select\n");
			break;
		}
	} else {
		...
		case GPU_MEM_CKSQ_MUX_CK:
			...
			pr_debug("mt_gpufreq_sel - GPU_MEM_CKSQ_MUX_CK\n");
			break;
		case GPU_MEM_TOP_SMI:
			clk_cfg = (clk_cfg & 0xfffcffff) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_8, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MEM_TOP_SMI\n");
			break;
		case GPU_MEM_TOP_MFG:
			clk_cfg = (clk_cfg & 0xfffcffff) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_8, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MEM_TOP_MFG IC ver %d\n", ver);
			break;
		case GPU_MEM_TOP_MEM:
			clk_cfg = (clk_cfg & 0xfffcffff) | (sel << 16);
			DRV_WriteReg32(CLK_CFG_8, clk_cfg);
			pr_debug("mt_gpufreq_sel - GPU_MEM_TOP_MEM\n");
			break;
		default:
			pr_err("mt_gpufreq_sel - not support mem sel\n");
			break;
		}
	}
}
...
static void mt_setup_gpufreqs_default_power_table(int num)
{
	...
	if (mt_gpufreqs_default_power == NULL) {
		pr_err("GPU default power table memory allocation fail\n");
		return;
	}
	...
	pr_debug("mt_gpufreqs_default_power[0]: gpufreq_khz = %u, gpufreq_power = %u\n",
		mt_gpufreqs_default_power[0].gpufreq_khz, mt_gpufreqs_default_power[0].gpufreq_power);
...
static int mt_setup_gpufreqs_table(struct mt_gpufreq_info *freqs, int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		pr_debug("freqs[%d].gpufreq_khz = %u\n", i, freqs[i].gpufreq_khz);
		pr_debug("freqs[%d].gpufreq_lower_bound = %u\n", i, freqs[i].gpufreq_lower_bound);
		pr_debug("freqs[%d].gpufreq_upper_bound = %u\n", i, freqs[i].gpufreq_upper_bound);
		pr_debug("freqs[%d].gpufreq_volt = %u\n", i, freqs[i].gpufreq_volt);
		pr_debug("freqs[%d].gpufreq_remap = %u\n", i, freqs[i].gpufreq_remap);
	}
	...
	pr_debug("mt_setup_gpufreqs_table, g_cur_freq = %d, g_cur_gpu_volt = %d\n",
...
int mt_gpufreq_state_set(int enabled)
{
	if (enabled) {
		if (!mt_gpufreq_pause) {
			pr_err("gpufreq already enabled\n");
			return 0;
		}
		...
		******************/
		...
		pr_debug("enable GPU DVFS: g_gpufreq_dvfs_disable_count = %d\n",
			    g_gpufreq_dvfs_disable_count);
		...
		************************************************/
		if (g_gpufreq_dvfs_disable_count <= 0)
			...
			pr_err("someone still disable gpufreq, cannot enable it\n");
	} else {
		/******************
		* disable GPU DVFS
		*******************/
		...
		pr_debug("disable GPU DVFS: g_gpufreq_dvfs_disable_count = %d\n",
			    g_gpufreq_dvfs_disable_count);
		if (mt_gpufreq_pause) {
			pr_err("gpufreq already disabled\n");
			return 0;
		}
...
static void mt_gpu_volt_switch(unsigned int volt_old, unsigned int volt_new)
{
	if (volt_old < volt_new) {
		pr_debug("mt_gpu_volt_switch: volt_old(%d) < volt_new(%d)\n", volt_old, volt_new);
	...
	if (volt_new == GPU_POWER_VGPU_1_15V_VOL) {
		...
		pr_debug("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VGPU_1_15V\n");
	} else if (volt_new == GPU_POWER_VGPU_1_10V_VOL) {
		mt_gpu_vgpu_sel_1_10();
		pr_debug("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VGPU_1_10V\n");
	} else if (volt_new == GPU_POWER_VGPU_1_05V_VOL) {
		mt_gpu_vgpu_sel_1_05();
		pr_debug("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VGPU_1_05V\n");
	} else if (volt_new == GPU_POWER_VGPU_1_025V_VOL) {
		mt_gpu_vgpu_sel_1_025();
		pr_debug("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VGPU_1_025V\n");
	} else if (volt_new == GPU_POWER_VGPU_1_00V_VOL) {
		mt_gpu_vgpu_sel_1_00();
		pr_debug("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VGPU_1_00V\n");
	} else if (volt_new == GPU_POWER_VGPU_0_95V_VOL) {
		mt_gpu_vgpu_sel_0_95();
		pr_debug("mt_gpu_volt_switch: switch MFG power to GPU_POWER_VGPU_0_95V\n");
	} else {
		pr_err("mt_gpu_volt_switch: do not support specified volt (%d)\n", volt_new);
		return;
	}
...
static void mt_gpu_volt_switch_initial(unsigned int volt_new)
{
	if (volt_new == GPU_POWER_VGPU_1_15V_VOL) {
		mt_gpu_vgpu_sel_1_15();
		pr_debug("mt_gpu_volt_switch_initial: switch MFG power to GPU_POWER_VGPU_1_15V\n");
	} else if (volt_new == GPU_POWER_VGPU_1_05V_VOL) {
		mt_gpu_vgpu_sel_1_05();
		pr_debug("mt_gpu_volt_switch_initial: switch MFG power to GPU_POWER_VGPU_1_05V\n");
	} else if (volt_new == GPU_POWER_VGPU_1_00V_VOL) {
		mt_gpu_vgpu_sel_1_00();
		pr_debug("mt_gpu_volt_switch_initial: switch MFG power to GPU_POWER_VGPU_1_00V\n");
	} else if (volt_new == GPU_POWER_VGPU_0_95V_VOL) {
		mt_gpu_vgpu_sel_0_95();
		pr_debug("mt_gpu_volt_switch_initial: switch MFG power to GPU_POWER_VGPU_0_95V\n");
	} else {
		pr_err("mt_gpu_volt_switch_initial: do not support specified volt (%d)\n",
			volt_new);
		return;
	}
...
static unsigned int mt_gpufreq_get_max_speed(void)
{
	...
	pr_debug("mt_gpufreq_get_max_speed m_hw_res4 is %d\n", m_hw_res4);
	m_hw_res4 = m_hw_res4 & (0x0000FFFF);

	/* Turbo with */
	if (0x1000 == m_hw_res4) {
		pr_debug("mt_gpufreq_get_max_speed:Turbo\n");
		return GPU_DVFS_F1;/* GPU_DVFS_OVER_F450 */;
	}
	/* MT8131 */
	else if (0x1003 == m_hw_res4) {
		pr_debug("mt_gpufreq_get_max_speed:MT8131\n");
		return GPU_DVFS_F1;
	}
	/* non-turbo PTPOD_SW_BOND  */
	else if (0x1001 == m_hw_res4) {
		pr_debug("mt_gpufreq_get_max_speed:non-turbo\n");
		/* update with new speed definition @ 2014/3/14 */
		return GPU_DVFS_F1;

		m_hw2_res4 = ((m_hw2_res4 & 0x00000700) >> 8);
		pr_debug("mt_gpufreq_get_max_speed:m_hw2_res4 is %d\n", m_hw2_res4);
		...
		pr_debug("mt_gpufreq_get_max_speed:default\n");
		return GPU_DVFS_F1;
	}
}
...
void mt_gpufreq_set_initial(void)
{
	...
	pr_debug("g_cur_gpu_freq = %d, before mt_gpufreq_set_initial\n",
		g_cur_gpu_freq);
	...
	pr_debug("mt_gpufreq_set_initial, g_cur_gpu_volt = %d, g_cur_gpu_freq = %d\n",
...
#ifdef abc123_DEVICE_TYPE
	/* HACK: Remove once abc123 devices are available */
	mt_gpufreqs[0].gpufreq_khz = GPU_DVFS_F3;
#else
...
static int mt_gpufreq_target(int idx)
{
    ...
    **********************************************/
	if (target_freq > mt_gpufreqs[g_limited_max_id].gpufreq_khz) {
		pr_debug("Limit! Target freq %d > Thermal limit frequency %d! adjusting...\n",
    ...
    *************************************************/
	if (g_cur_gpu_freq == target_freq) {
		...
		pr_debug("GPU frequency from %d MHz to %d MHz (skipped) due to same frequency\n",
			g_cur_gpu_freq / 1000, target_freq / 1000);
		return 0;
	}

	pr_debug("GPU current frequency %d MHz, target frequency %d MHz\n",
		g_cur_gpu_freq / 1000, target_freq / 1000);
...
{
	pr_debug("mt_gpufreq_early_suspend: g_cur_gpu_volt is %d, g_cur_gpu_freq is %d\n",
		g_cur_gpu_volt, g_cur_gpu_freq);
...
{
	pr_debug("mt_gpufreq_late_resume: g_cur_gpu_volt is %d, g_cur_gpu_freq is %d\n",
		g_cur_gpu_volt, g_cur_gpu_freq);
...
void mt_gpufreq_thermal_protect(unsigned int limited_power)
{
	...
	pr_debug("limit frequency upper bound to id = %d, frequency = %d\n",
				      ...
				      loff_t *data)
{
	...
	if ((mt_gpufreq_registered == false) && (mt_gpufreq_registered_statewrite == false)) {
		...
		pr_debug("freqs[0].gpufreq_khz = %u\n", mt_gpufreqs[0].gpufreq_khz);
		pr_debug("freqs[0].gpufreq_lower_bound = %u\n", mt_gpufreqs[0].gpufreq_lower_bound);
		pr_debug("freqs[0].gpufreq_upper_bound = %u\n", mt_gpufreqs[0].gpufreq_upper_bound);
		pr_debug("freqs[0].gpufreq_volt = %u\n", mt_gpufreqs[0].gpufreq_volt);
		pr_debug("freqs[0].gpufreq_remap = %u\n", mt_gpufreqs[0].gpufreq_remap);
					 ...
					 loff_t *data)
{
	...
	if (sscanf(buffer, "%d %d", &set_freq, &set_volt) == 2) {
		...
		pr_err("bad argument!! should be [set_freq set_volt] as [300000 1150]\n");
	}
...
int mt_gpufreq_switch_handler(unsigned int load)
{
	...
	pr_debug("mt_gpufreq_switch_handler() idx is %d\n", idx);
...
int mt_gpufreq_register(struct mt_gpufreq_info *freqs, int num)
{
	if (mt_gpufreq_registered == false) {
		...
		pr_debug("mediatek gpufreq registration done\n");
	} else {
		pr_err("mediatek gpufreq already registered !\n");
	}
...
int mt_gpufreq_non_register(void)
{
	if (mt_gpufreq_already_non_registered == false) {
		...
		pr_debug("mt_gpufreq_non_register() done\n");
	} else {
		pr_err("mt_gpufreq_non_register() already called !\n");
	}
	return 0;
}
...
static int __init mt_gpufreq_init(void)
{
	...
	if (!mt_gpufreq_dir)
		...
		mt_entry =
		    proc_create("gpufreq_debug", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_debug_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq_debug failed\n");

		mt_entry =
		    proc_create("gpufreq_limited_power", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_limited_power_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq_limited_power failed\n");

		mt_entry =
		    proc_create("gpufreq_state", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_state_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq_state failed\n");

		mt_entry =
		    proc_create("gpufreq_opp_dump", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_opp_dump_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq_opp_dump failed\n");

		mt_entry =
		    proc_create("gpufreq_set_freq", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_set_freq_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq_set_freq failed\n");

		mt_entry =
		    proc_create("gpufreq_fix_frequency", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_fixed_frequency_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq_fix_frequency failed\n");

		mt_entry =
		    proc_create("gpufreq_var_dump", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_var_dump_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq_var_dump failed\n");

		mt_entry =
			proc_create("gpufreq", S_IRUGO | S_IWUSR | S_IWGRP,
				mt_gpufreq_dir, &mt_gpufreq_fops);
		if (!mt_entry)
			pr_err("mkdir /proc/gpufreq/gpufreq failed\n");
	}

	clk_cfg_0 = DRV_Reg32(CLK_CFG_0);
	clk_cfg_0 = (clk_cfg_0 & 0x000F0000) >> 16;
	pr_notice("mt_gpufreq_init, clk_cfg_0 = %d\n", clk_cfg_0);
