...
static DEFINE_MUTEX(msdc4sdio3_mutex);
...
#define MSDC_SDIO_FLAG    (MSDC_EXT_SDIO_IRQ | MSDC_HIGHSPEED | MSDC_UHS1 | MSDC_REMOVABLE)
#else
...
static const struct mmcdev_info msdc1_devinfo[] = {
	{
		.r_smpl = MSDC_SMPL_RISING, .wd_smpl = MSDC_SMPL_RISING, .d_smpl = MSDC_SMPL_RISING,
	},
};
struct msdc_hw msdc1_hw = {
	...
	.max_busy_time = 3000,
	.poll_sleep_base = 0, /* set to 0 to prevent sleep-in-atomic */
	.clk_src = MSDC_CLKSRC_200MHZ,
	.dev_info = msdc1_devinfo,
	.dev_info_num = ARRAY_SIZE(msdc1_devinfo),
	.pin_ctl = {
		{
			.voltage = BIT(MMC_SIGNAL_VOLTAGE_180),
			.clk_drv = 6,
			.cmd_drv = 6,
			.dat_drv = 6,
		},
