...
#define CPU_COOLER "cpu_adaptive"
#define MASK (0x0FFF)

static int cpu_tz_match(struct thermal_zone_device *tz,
		struct thermal_cooling_device *cdev)
{
	...
	if (!strncmp(cdev->type, CPU_COOLER, strlen(CPU_COOLER)))
		return 0;
...
static struct thermal_bind_params tbp[] = {
	{.cdev = NULL, .weight = 0,
	.trip_mask = MASK, .match = cpu_tz_match},
};
...
static struct mtk_thermal_platform_data mtktscpu_thermal_data = {
	...
	.polling_delay = 100,
	.tzp = {
		.governor_name = DEFAULT_THERMAL_GOVERNOR,
		.num_tbps = 1,
		.tbp = tbp,
	},
	.trips[0] = {.temp = 85000, .type = THERMAL_TRIP_ACTIVE, .hyst = 0},
	.trips[1] = {.temp = MTKTSCPU_TEMP_CRIT, .type = THERMAL_TRIP_CRITICAL, .hyst = 0},
};
...
static struct mtk_cooler_platform_data mtk_cpu_adaptive_cooler_data = {
	.type = "cpu_adaptive",
	.state = 0,
	.max_state = 1,
};

struct platform_device cooler_pdev = {
	.name = "mtk_cpu_adaptive_cooler",
	.id = -1,
	.dev = {
		.platform_data = &mtk_cpu_adaptive_cooler_data,
	},
};
...
/* FIXME: need porting */
	...
	if (reserved_mem_bank_tag) {
		...
		/* reserve frame buffer use memblock_reserve */
		if (g_boot_mode == NORMAL_BOOT) {
			reserved_mem_bank_tag->u.mem.size += ((__u32)RESERVED_MEM_SIZE_FOR_FB);
			mi->bank[mi->nr_banks - 1].size += ((__u32)RESERVED_MEM_SIZE_FOR_FB);
...
/* spi_register_board_info(spi_board_devs, ARRAY_SIZE(spi_board_devs)); */
	...
	retval = platform_device_register(&cooler_pdev);
	if (retval != 0)
		return retval;
