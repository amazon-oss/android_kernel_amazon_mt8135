...
mt_battery_charging_custom_data mt_bat_charging_data = {
	...
	/* Battery Charge Temperature Detection*/
	.max_discharge_temp = 60,
	.min_discharge_temp = -10,
	...
	.tdata = {
		.num_trips = 1,
		.trips[0] = {.temp = BATTERY_TEMP_CRIT, .type = THERMAL_TRIP_CRITICAL, .hyst = 0},
...
static struct platform_device battery_device = {
	...
	.id = -1,
	.dev = {
		.platform_data = &mt_bat_charging_data_wrapper}
};
...
void __init mt_battery_init(void)
{
	...
	mt_bat_charging_data_wrapper.data = &mt_bat_charging_data;
