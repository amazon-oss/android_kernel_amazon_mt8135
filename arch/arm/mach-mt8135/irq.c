...
static unsigned int get_pol_status(int irq)
{
	unsigned int bit;
	bit = 1 << (irq % 32);
	return (readl(IOMEM(INT_POL_CTL0 + irq / 32 * 4)) & bit) ? 1 : 0;
}
...
void mt_irq_dump_status(int irq)
{
	if (irq >= NR_IRQS)
		return;
	pr_err("[INT] irq:%d,enable:%x,active:%x,pending:%x,pri:%x,grp:%x,target:%x,sens:%x, pol:%x\n",
		irq, get_mask(irq), get_active_status(irq), get_pending_status(irq), get_pri(irq),
		get_grp(irq), get_target(irq), get_sens(irq), get_pol_status(irq));
